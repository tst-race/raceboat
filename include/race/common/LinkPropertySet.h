...
public:
  /**
   * @brief Bandwidth measured in bits per second. If the value is negative then
   * the value is either unset or unknown for this link.
   *
   */
  std::int32_t bandwidth_bps;
  /**
   * @brief latency measured in milliseconds, includes estimated Comms
   * processing time on both sides. If the value is negative then the value is
   * either unset or unknown for this link.
   *
   */
  std::int32_t latency_ms;
  /**
   * @brief loss measured in chance-of-loss per-unit. If the value is negative
   * then the value is either unset or unknown for this link.
   *
   */
  float loss;
};
...
inline bool operator==(const LinkPropertySet &a, const LinkPropertySet &b) {
  return a.bandwidth_bps == b.bandwidth_bps && a.latency_ms == b.latency_ms &&
         a.loss == b.loss;
}
inline bool operator!=(const LinkPropertySet &a, const LinkPropertySet &b) {
  return a.bandwidth_bps != b.bandwidth_bps || a.latency_ms != b.latency_ms ||
...
/**
 * @brief Convert a LinkPropertySet object to a human-readable string. This
