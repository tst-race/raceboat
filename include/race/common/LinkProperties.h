...
public:
  ...
  /**
   * @brief Should be set to true if the link supports being flushed by
  ...
  /**
   * @brief The amount of time (in seconds) the link must wait between
  ...
  /**
   * @brief LinkAddress of this link - used by loadLinkAddress to establish a
   * link with this side
   *
   */
...
inline bool operator==(const LinkProperties &a, const LinkProperties &b) {
  return a.linkType == b.linkType && a.channelGid == b.channelGid &&
