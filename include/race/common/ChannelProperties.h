...
public:
  ...
  /**
   * @brief If the channel supports the loadLinkAddresses API (multiple
   * addresses)
   *
   */
  ...
  /**
   * @brief Should be set to true if the channel supports being flushed by
   * NetworkManager, otherwise false.
   *
   */
  bool isFlushable;
  ...
  /**
   * @brief The amount of time (in seconds) the channel must wait between
   * activations (inclusive of duration)
   *
   */
   ...
   *
   * If this channel does not have a sends-per-interval constraint, this value
   * will be -1.
   */
...
inline bool operator==(const ChannelProperties &a, const ChannelProperties &b) {
  return a.linkDirection == b.linkDirection && a.channelGid == b.channelGid &&
         ...
         a.intervalEndTime == b.intervalEndTime &&
...
/**
 * @brief Convert a ChannelProperties object to a human readable string. This
...
/**
 * @brief Compare two ChannelProperties objects and determine if the static
 * properties are equal to each other. Properties are defined as static or
 * dynamic in this document:
