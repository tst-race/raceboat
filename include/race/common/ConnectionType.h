...
enum ConnectionType {
  ...
  CT_MIXED = 3,    // link both reveals its IP to other node and contacts a
                   // 3rd-party service
  CT_LOCAL = 4     // link is physically local (e.g. a WiFi hotspot)
};

/**
 * @brief Convert a ConnectionType value to a human readable string. This
...
// TODO: documentation
ConnectionType
connectionTypeFromString(const std::string &connectionTypeString);
