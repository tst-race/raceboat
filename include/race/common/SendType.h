...
enum SendType {
  ...
  ST_STORED_ASYNC = 1, // sent messages are stored but there is no way to know
                       // the other side is ready
  ST_EPHEM_SYNC =
      2 // the other side must be ready, but the link will know when that occurs
};

/**
 * @brief Convert a SendType value to a human readable string. This function is
