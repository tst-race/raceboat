...
public:
   ...
   * @param pluginConfig Config object containing dynamic config variables (e.g.
   * paths)
   ...
   *
   * @param handle The handle for asynchronously returning the status of this
   * call.
   * @param connectionId The ID of the connection to use to send the package.
   * @param pkg The encrypted package to send.
   * @param timeoutTimestamp The time the package must be sent by. Measured in
   * seconds since epoch
   * @param batchId The batch ID used to group encrypted packages so that they
   * can be flushed at the same time when flushChannel is called. If this value
   * is zero then it can safely be ignored.
   * @return PluginResponse the status of the Plugin in response to this call
   */
  ...
  /**
   * @brief Open a connection with a given type on the specified link.
   * Additional configuration info can be provided via the linkHints param.
   *
   * @param handle The handle for asynchronously returning the status of this
   * call.
   ...
   * @param linkHints Additional optional configuration information provided by
   * NetworkManager as a stringified JSON Object.
   * @return PluginResponse the status of the Plugin in response to this call
   */
  ...
  /**
   * @brief Load a link of the specified channel using the provided
   * LinkAddresses
   *
   * @param handle The handle for asynchronously returning the status of this
   * call.
   ...
   */
  virtual PluginResponse createLinkFromAddress(RaceHandle handle,
                                               std::string channelGid,
                                               std::string linkAddress) = 0;

  /**
   * @brief Deactivate the specified channel, destroying all associated links
   * and closing all associated connections, and setting the channel to
   * unavailable
   *
   * @param handle The handle for asynchronously returning the status of this
   * call.
   ...
   * @param handle The handle for asynchronously returning the status of this
   * call.
   * @param answered True if the response contains an actual answer to the input
   * prompt, otherwise the response is an empty string and not valid
  ...
  /**
   * @brief Notify the plugin that the user acknowledged the displayed
   * information
   *
   * @param handle The handle for asynchronously returning the status of this
   * call.
  ...
  /**
   * @brief Serve files located in the specified directory. The files served are
   * associated with the specified link and may stop being served when the link
   * is closed.
   *
  ...
  /**
   * @brief Create a bootstrap link of the channel specified with the specified
   * passphrase
   *
   * @param handle The handle for asynchronously returning the status of this
   * call.
   ...
   * @return PluginResponse the status of the Plugin in response to this call
   */
  virtual PluginResponse createBootstrapLink(RaceHandle /*handle*/,
                                             std::string /*channelGid*/,
  ...
  /**
   * @brief Flush any pending encrypted packages queued to be sent out over the
   * given channel.
   * @param handle The handle for asynchronously returning the status of this
   * call.
   * @param channelGid The ID of the channel to flush.
   * @param batchId The batch ID of the encrypted packages to be flushed. If
   * batch ID is set to zero (the null value) this call will return an error. A
   * valid batch ID must be provided.
   * @return PluginResponse Status of plugin indicating success or failure of
   * the call.
   */
