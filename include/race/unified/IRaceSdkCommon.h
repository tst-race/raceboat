...
public:
  ...
  /**
   * @brief Query the system for entropy. Entropy will be gathered from system
   * calls as well as external sources, e.g. user scribbling on screen or
   * reading from a device's microphone.
   *
   * @param numBytes
   * @return RawData
   */
   ...
   *        should not change, but we are building for the possibility it will
   * in the future.
   *
  ...
  listDir(const std::string &directoryPath) = 0;
  ...
  /**
   * @brief Write the contents of data to filename in this plugin's storage
   * (overwriting if file exists). Directories are created as-needed.
