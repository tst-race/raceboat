...
public:
  ...
  /**
   * @brief Notify NetworkManager via the SDK that the status of this package
   * has changed.
   *
   * @param handle The RaceHandle identifying the sendEncryptedPackage call it
   * corresponds to
   * @param status The new PackageStatus
   * @param timeout Timeout in milliseconds to block, 0 indicates a nonblocking
   * call, negative values are invalid arguments
   * @return SdkResponse the status of the SDK in response to this call
   */
  virtual SdkResponse onPackageStatusChanged(RaceHandle handle,
                                             PackageStatus status,
                                             int32_t timeout) = 0;

  /**
   * @brief Notify NetworkManager via the SDK that the stauts of a connection
   * has changed
   *
   * @param handle The RaceHandle identifying the NetworkManager call it
   * corresponds to
   * @param status The new ConnectionStatus
   * @param timeout Timeout in milliseconds to block, 0 indicates a nonblocking
   * call. The value RACE_BLOCKING indicates the call will block forever. Other
   * negative values are invalid arguments
   * @return SdkResponse the status of the SDK in response to this call
   */
  virtual SdkResponse onConnectionStatusChanged(RaceHandle handle,
                                                ConnectionID connId,
  ...
  /**
   * @brief Notify NetworkManager via the SDK that the stauts of a channel has
   * changed
   *
   * @param handle The RaceHandle identifying the NetworkManager call it
   ...
   * @return SdkResponse the status of the SDK in response to this call
   */
  virtual SdkResponse onChannelStatusChanged(RaceHandle handle,
  ...
  /**
   * @brief Notify NetworkManager via the SDK that the stauts of a link has
   * changed
   *
   * @param handle The RaceHandle identifying the NetworkManager call it
   * corresponds to (if any)
   ...
   * @return SdkResponse the status of the SDK in response to this call
   */
  virtual SdkResponse onLinkStatusChanged(RaceHandle handle, LinkID linkId,
                                          LinkStatus status,
                                          LinkProperties properties,
                                          int32_t timeout) = 0;
   ...
   * @return SdkResponse the status of the SDK in response to this call
   */
  virtual SdkResponse updateLinkProperties(LinkID linkId,
                                           LinkProperties properties,
                                           int32_t timeout) = 0;
   ...
   * @return SdkResponse the status of the SDK in response to this call
   */
  virtual SdkResponse receiveEncPkg(const EncPkg &pkg,
                                    const std::vector<ConnectionID> &connIDs,
                                    int32_t timeout) = 0;

  /**
   * @brief Request plugin-specific input from the user with the specified
   * prompt message. The response may be cached in persistent storage, in which
   * case the user will not be re-prompted if a cached response exists for the
   * given prompt.
   *
   * The response will be provided in the userInputReceived callback with the
   ...
   */
  virtual SdkResponse requestPluginUserInput(const std::string &key,
                                             const std::string &prompt,
                                             bool cache) = 0;

  /**
   * @brief Request application-wide input from the user associated with the
   * given key. The key identifies a common user input prompt and must be a key
   * supported by the RACE SDK. The response is cached in persistent storage, so
   * the user will not be re-prompted if a cached response exists for the given
   * key.
   *
   * The response will be provided in the userInputReceived callback with the
   * handle matching the handle returned in this SdkResponse object.
   ...
   *
   * The task posted to the work queue will display information to the user
   * input prompt, wait an optional amount of time, then notify the SDK of the
   * user acknowledgment.
   *
   ...
   */
  virtual SdkResponse
  displayInfoToUser(const std::string &data,
                    RaceEnums::UserDisplayType displayType) = 0;

  /**
   * @brief Displays information to the User and forward information to target
   * node for automated testing
   *
   * @param data data to display
   * @param displayType type of user display to display data in
   ...
   */
  virtual SdkResponse
  displayBootstrapInfoToUser(const std::string &data,
                             RaceEnums::UserDisplayType displayType,
                             RaceEnums::BootstrapActionType actionType) = 0;

  /**
   * @brief Unblock the queue for a connection previously blocked by a return
   * value of PLUGIN_TEMP_ERROR.
   *
