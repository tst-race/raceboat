...
echo "Building raceboat package"
PACKAGE_DIR="${DIR}/racesdk/package"
mkdir -p "${PACKAGE_DIR}"
...
HELP=\
"This script prepares the RACE sdk artifacts to be packaged for use in the raceboat
docker image.
...
if $PACKAGE_ANDROID ; then
  formatlog "Packaging ANDROID_x86_64"
  cmake --install build/ANDROID_x86_64/ --component sdk --verbose
fi

if $PACKAGE_ANDROID_ARM64 ; then
  formatlog "Packaging ANDROID_arm64"
  cmake --install build/ANDROID_arm64-v8a/ --component sdk --verbose
fi

if $PACKAGE_LINUX ; then
  formatlog "Packaging LINUX_x86_64"
  cmake --install build/LINUX_x86_64/ --component sdk --verbose
fi

if $PACKAGE_LINUX_ARM64 ; then
  formatlog "Packaging LINUX_arm64"
  # cmake --install racesdk/package/LINUX_arm64-v8a --component sdk --verbose
  cmake --install build/LINUX_arm64-v8a --component sdk --verbose
fi

formatlog "package dir ${PACKAGE_DIR}"
