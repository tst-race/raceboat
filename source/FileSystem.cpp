...
bool FileSystem::appendFile(const fs::path &filePath, const std::string &pluginId,
...
bool FileSystem::makeDir(const fs::path &directoryPath, const std::string &pluginId) {
...
bool FileSystem::removeDir(const fs::path &directoryPath, const std::string &pluginId) {
    TRACE_METHOD(directoryPath, pluginId);
    bool success = false;
    fs::path fullPath = makePluginFilePath(directoryPath, pluginId);
    std::error_code ec;
...
bool FileSystem::copy(const fs::path &srcPath, const fs::path &destPath) {
    ...
    if (!fs::exists(srcPath)) {
        success = false;
        helper::logInfo(logPrefix + " source path does not exist: " + srcPath.string());
    } else {
        try {
...
bool FileSystem::FileSystem::writeFile(const fs::path &filePath, const std::string &pluginId,
                                       const std::vector<std::uint8_t> &data) {
    TRACE_METHOD(filePath, pluginId);
    ...
    if (!path.has_parent_path()) {
        ...
        helper::logWarning(logPrefix + " path has no parent path: " + path.string());
    } else if (!fs::exists(path.parent_path())) {
        helper::logDebug(logPrefix + " parent path does not exist.  Creating parent path: " +
...
fs::path FileSystem::makePluginFilePath(const fs::path &filePath, const std::string &pluginId) {
    fs::path path = pluginsInstallPath / "usr" / pluginId;  // throws exception if pluginId empty
...
fs::path FileSystem::makePluginInstallPath(const fs::path &filePath, const std::string &pluginId) {
    fs::path path = makePluginInstallBasePath() / pluginId;  // throws exception if pluginId empty
    if (!filePath.empty()) {                                 // would throw exception otherwise
        path = path / filePath;
    }
    return path;
...
fs::path FileSystem::makePluginInstallBasePath() {
    fs::path basePath = pluginsInstallPath / "plugins" / getHostOsType() / getHostArch();
    createDirectories(basePath);
    return basePath;
}

fs::path FileSystem::makeRaceDir(const fs::path &prefix, const std::string &pluginId) {
