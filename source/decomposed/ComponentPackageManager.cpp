...
// How many bytes are used to store the length of the package for
// batched/fragmented messages
static const size_t FRAGMENT_LEN_SIZE = 4;

ComponentPackageManager::ComponentPackageManager(
    ComponentManagerInternal &manager)
    : manager(manager) {}

bool ComponentPackageManager::isLastFragment(
    CMTypes::PackageFragmentInfo *fragment) {
...
size_t ComponentPackageManager::spaceAvailableInAction(ActionInfo *actionInfo) {
  ...
  if (manager.mode == EncodingMode::FRAGMENT_SINGLE_PRODUCER) {
    ...
    helper::logError(logPrefix +
                     "Unknown mode: " + encodingModeToString(manager.mode));
    return 0;
  }
...
bool ComponentPackageManager::isPackageAbleToFit(Link *link,
                                                 const EncPkg &pkg) {
  for (auto actionInfo : link->actionQueue) {
    bool validLink =
        ((actionInfo->wildcardLink && actionInfo->linkId.empty()) ||
...
bool ComponentPackageManager::isTimeToEncode(double now,
...
bool ComponentPackageManager::generateFragmentsForPackage(
    double now, Link *link, PackageInfo *packageInfo) {
...
PluginResponse ComponentPackageManager::sendPackage(
    ComponentWrapperHandle /* postId */, double now, PackageSdkHandle handle,
...
CMTypes::CmInternalStatus ComponentPackageManager::onLinkStatusChanged(
    CMTypes::ComponentWrapperHandle /* postId */,
  ...
  try {
    if (status == LINK_DESTROYED) {
      ...
      // remove link fragments from global fragments and notify networkManager
      // of failed packages
      for (auto &packageInfo : link->packageQueue) {
        // have to inform networkManager now. It's possible this call causes a
        // queued onPackageStatusChanged call to fail due to missing fragments.
        // It's also possible that that call succeeded and this package was
        // actually sent out.
...
bool ComponentPackageManager::isActionEncoded(CMTypes::ActionInfo *actionInfo) {
  return std::any_of(
      actionInfo->encoding.begin(), actionInfo->encoding.end(),
...
CMTypes::CmInternalStatus ComponentPackageManager::onBytesEncoded(
  ...
  if (iter == pendingEncodings.end()) {
    helper::logInfo(logPrefix + "No pending encodings found, action may have "
                                "been canceled or already executed");
    return OK;
  }
  ...
  if (status == ENCODE_OK) {
    manager.getTransport()->enqueueContent(
        pendingEncoding->params, pendingEncoding->info->action, bytes);
...
CMTypes::CmInternalStatus ComponentPackageManager::onPackageStatusChanged(
    ComponentWrapperHandle /* postId */, PackageFragmentHandle handle,
  ...
  if (fragmentIter != fragments.end()) {
    ...
    if (isPackageFinished(pkg)) {
      ...
      manager.sdk.onPackageStatusChanged(pkg->sdkHandle.handle, status,
                                         RACE_BLOCKING);
      link->packageQueue.erase(
          std::remove_if(link->packageQueue.begin(), link->packageQueue.end(),
                         [&pkg](const auto &queuePkg) {
                           return pkg->sdkHandle == queuePkg->sdkHandle;
                         }),
    ...
    helper::logDebug(logPrefix + "Unable to find fragment with handle: " +
                     std::to_string(handle.handle));
  }
...
void ComponentPackageManager::generateFragmentsForAllPackages() {
  ...
  // remove packages
  // TODO: only remove packages if an action has been inserted or removed, or if
  // a package has been removed from any of the links
  for (auto &link : links) {
    for (auto actionInfo : link->actionQueue) {
  ...
  for (auto &encodingInfo : actionInfo->encoding) {
    ...
    if (iter != pendingEncodings.end()) {
      helper::logWarning(logPrefix +
                         "Action completed while encoding is still pending, "
                         "encoding handle: " +
...
std::vector<CMTypes::PackageFragmentHandle>
ComponentPackageManager::getPackageHandlesForAction(
  ...
  std::transform(actionInfo->fragments.begin(), actionInfo->fragments.end(),
                 ...
                 [](const auto &frag) { return frag->handle; });
  return handles;
}
...
void ComponentPackageManager::setup() { TRACE_METHOD(); }

static std::ostream &operator<<(
    std::ostream &out,
    const std::unordered_map<CMTypes::EncodingHandle, CMTypes::EncodingInfo *>
        &pendingEncodings) {
...
static std::ostream &operator<<(
    std::ostream &out,
    const std::unordered_map<CMTypes::PackageFragmentHandle,
                             CMTypes::PackageFragmentInfo *> &fragments) {
...
std::ostream &operator<<(std::ostream &out,
                         const ComponentPackageManager &manager) {
