...
ComponentActionManager::ComponentActionManager(
    ...
    : nextFetchTime(0), nextActionTime(std::numeric_limits<double>::infinity()),
      nextEncodeTime(std::numeric_limits<double>::infinity()),
      lastEncodeTime(0), manager(manager) {}
...
double ComponentActionManager::getMaxEncodingTime() { return maxEncodingTime; }
  ...
  try {
    if (status == LINK_DESTROYED) {
      ...
      actions.erase(std::remove_if(actions.begin(), actions.end(),
                                   [set](auto &actionInfo) {
                                     return set.count(actionInfo.get()) > 0;
                                   }),
                    actions.end());
...
ComponentActionManager::onSendPackage(double now, const ConnectionID &connId,
                                      const EncPkg &pkg) {
  auto linkId = manager.getConnection(connId)->linkId;
  auto usermodelActions =
      manager.getUserModel()->onSendPackage(linkId, pkg.getSize());
...
CMTypes::CmInternalStatus
ComponentActionManager::onTimelineUpdated(ComponentWrapperHandle /* postId */) {
  fetchTimeline();
  return OK;
}
...
ComponentActionManager::createActionInfo(Action &&action) {
  ...
  for (auto &param : params) {
    if (param.encodePackage) {
      ...
      // TODO: if this is for cover trafic, we will never select the actual
      // link. Does that matter, or can we let the transport decide?
...
void ComponentActionManager::updateGlobalTimeline(
    ActionTimeline &usermodelActions, Timestamp start) {
  std::deque<std::unique_ptr<ActionInfo>> newActions;

  auto oldIt = actions.begin();

  // advance iterator until after start. We remove actions that the usermodel
  // didn't return, unless they happened before start.
...
void ComponentActionManager::runActionThread() {
  ...
  while (true) {
    ...
    helper::logDebug(logPrefix + "Sleeping for " +
                     std::to_string(wait_until - start) + " seconds");
    std::chrono::high_resolution_clock::time_point tp{
        std::chrono::microseconds{long(1000000 * wait_until)}};
    actionThreadSignaler.wait_until(lock, tp);
    helper::logDebug(logPrefix + "Woke up after " +
                     std::to_string(currentTime() - start) + " seconds");
...
void ComponentActionManager::doAction() {
  ...
  if (info->wildcardLink) {
    ...
    try {
      ...
      // Link doesn't exist. This should never happen as the action should be
      // deleted if the link is removed.
...
void ComponentActionManager::encodeActions(Timestamp now) {
  ...
  while (it != actions.end() &&
         (*it)->action.timestamp < now + maxEncodingTime) {
    manager.encodeForAction(it->get());
    lastEncodeTime = (*it)->action.timestamp;
    it++;
  }

  // set next encode time based on timestamp of the next action, or inf if there
  // is none
  nextEncodeTime = std::numeric_limits<double>::infinity();
  if (it != actions.end()) {
    nextEncodeTime = (*it)->action.timestamp - maxEncodingTime;
...
double ComponentActionManager::currentTime() { return helper::currentTime(); }

static std::string
printActions(const std::deque<std::unique_ptr<CMTypes::ActionInfo>> &deque) {
  std::stringstream ss;
  ss << "[";
  for (auto &e : deque) {
    ss << *e << ", ";
