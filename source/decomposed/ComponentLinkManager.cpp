...
ComponentLinkManager::ComponentLinkManager(ComponentManagerInternal &manager)
    : manager(manager) {}

CMTypes::CmInternalStatus
ComponentLinkManager::destroyLink(ComponentWrapperHandle /* postId */,
...
CMTypes::CmInternalStatus
ComponentLinkManager::createLink(ComponentWrapperHandle /* postId */,
...
CMTypes::CmInternalStatus ComponentLinkManager::onLinkStatusChanged(
  ...
  if (status == LINK_CREATED || status == LINK_LOADED) {
    ...
    // create a copy of connections so modifying the orginal doesn't cause
    // problems
  ...
  manager.sdk.onLinkStatusChanged(handle.handle, linkId, status, props,
                                  RACE_BLOCKING);
  return OK;
}
...
void ComponentLinkManager::setup() { TRACE_METHOD(); }
} // namespace Raceboat
