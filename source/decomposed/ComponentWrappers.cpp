...
std::ostream &operator<<(std::ostream &out,
                         const ComponentBaseWrapper &wrapper) {
  return out << wrapper.toString();
}

template <typename ComponentType>
template <typename T, typename... Args>
void TemplatedComponentWrapper<ComponentType>::post(
    const std::string &logPrefix, T &&function, Args &&... args) {
  ...
  try {
    auto workFunc = [logPrefix, postId, function,
      ...
      helper::logDebug(logPrefix + "Returned " +
                       componentStatusToString(response) +
                       ", postId: " + postId);
...
ReturnType TemplatedComponentWrapper<ComponentType>::post_sync(
    const std::string &logPrefix, T &&function, Args &&... args) {
  std::string postId = std::to_string(nextPostId++);
  helper::logDebug(logPrefix + "Posting postId: " + postId);
    ...
    auto workFunc = [logPrefix, postId, function,
      ...
      try {
        return std::make_optional(
            std::mem_fn(function)(*component, std::move(args)...));
      } catch (std::exception &e) {
        helper::logError(logPrefix +
                         "Threw exception: " + std::string(e.what()));
        manager->markFailed();
...
TemplatedComponentWrapper<ComponentType>::TemplatedComponentWrapper(
    ...
    : handler(channelName + "-" + componentName + "-thread", 1 << 20, 1 << 20),
      component(std::move(component)), manager(manager) {
...
void TemplatedComponentWrapper<ComponentType>::onUserInputReceived(
    CMTypes::UserComponentHandle handle, bool answered,
    const std::string &response) {
  TRACE_METHOD(handle, answered, response);
  post(logPrefix, &IComponentBase::onUserInputReceived, handle.handle, answered,
       response);
}
...
TransportComponentWrapper::TransportComponentWrapper(
    const std::string &channelName, const std::string &componentName,
    std::shared_ptr<ITransportComponent> component, ComponentManager *manager)
    : TemplatedComponentWrapper(channelName, componentName, component,
                                manager) {}

TransportProperties TransportComponentWrapper::getTransportProperties() {
  TRACE_METHOD();
  return post_sync<TransportProperties>(
      logPrefix, &ITransportComponent::getTransportProperties);
}

LinkProperties
TransportComponentWrapper::getLinkProperties(const LinkID &linkId) {
  TRACE_METHOD(linkId);
  return post_sync<LinkProperties>(
      logPrefix, &ITransportComponent::getLinkProperties, linkId);
}

void TransportComponentWrapper::createLink(CMTypes::LinkSdkHandle handle,
                                           const LinkID &linkId) {
  TRACE_METHOD(handle, linkId);
  post(logPrefix, &ITransportComponent::createLink, handle.handle, linkId);
}

void TransportComponentWrapper::loadLinkAddress(
    CMTypes::LinkSdkHandle handle, const LinkID &linkId,
    const std::string &linkAddress) {
  TRACE_METHOD(handle, linkId, linkAddress);
  post(logPrefix, &ITransportComponent::loadLinkAddress, handle.handle, linkId,
       linkAddress);
}

void TransportComponentWrapper::loadLinkAddresses(
    CMTypes::LinkSdkHandle handle, const LinkID &linkId,
    const std::vector<std::string> &linkAddress) {
  TRACE_METHOD(handle, linkId, linkAddress.size());
  post(logPrefix, &ITransportComponent::loadLinkAddresses, handle.handle,
       linkId, linkAddress);
}

void TransportComponentWrapper::createLinkFromAddress(
    CMTypes::LinkSdkHandle handle, const LinkID &linkId,
    const std::string &linkAddress) {
  TRACE_METHOD(handle, linkId, linkAddress);
  post(logPrefix, &ITransportComponent::createLinkFromAddress, handle.handle,
       linkId, linkAddress);
}

void TransportComponentWrapper::destroyLink(CMTypes::LinkSdkHandle handle,
                                            const LinkID &linkId) {
  TRACE_METHOD(handle, linkId);
  post(logPrefix, &ITransportComponent::destroyLink, handle.handle, linkId);
}

std::vector<EncodingParameters>
TransportComponentWrapper::getActionParams(const Action &action) {
  TRACE_METHOD(action);
  return post_sync<std::vector<EncodingParameters>>(
      logPrefix, &ITransportComponent::getActionParams, action);
}

void TransportComponentWrapper::enqueueContent(
    const EncodingParameters &params, const Action &action,
    const std::vector<uint8_t> &content) {
  TRACE_METHOD(params, action, content.size());
  post(logPrefix, &ITransportComponent::enqueueContent, params, action,
       content);
}
...
void TransportComponentWrapper::doAction(
    const std::vector<CMTypes::PackageFragmentHandle> &handles,
    const Action &action) {
...
UserModelComponentWrapper::UserModelComponentWrapper(
    const std::string &channelName, const std::string &componentName,
    std::shared_ptr<IUserModelComponent> component, ComponentManager *manager)
    : TemplatedComponentWrapper(channelName, componentName, component,
                                manager) {}

UserModelProperties UserModelComponentWrapper::getUserModelProperties() {
  TRACE_METHOD();
  return post_sync<UserModelProperties>(
      logPrefix, &IUserModelComponent::getUserModelProperties);
}

void UserModelComponentWrapper::addLink(const LinkID &linkId,
...
ActionTimeline UserModelComponentWrapper::getTimeline(Timestamp start,
                                                      Timestamp end) {
  TRACE_METHOD(start, end);
  return post_sync<ActionTimeline>(logPrefix, &IUserModelComponent::getTimeline,
                                   start, end);
}
...
ActionTimeline UserModelComponentWrapper::onSendPackage(const LinkID &linkId,
                                                        int bytes) {
  TRACE_METHOD(linkId, bytes);
  return post_sync<ActionTimeline>(
      logPrefix, &IUserModelComponent::onSendPackage, linkId, bytes);
}
...
EncodingComponentWrapper::EncodingComponentWrapper(
    const std::string &channelName, const std::string &componentName,
    std::shared_ptr<IEncodingComponent> component, ComponentManager *manager)
    : TemplatedComponentWrapper(channelName, componentName, component,
                                manager) {}

EncodingProperties EncodingComponentWrapper::getEncodingProperties() {
  TRACE_METHOD();
  return post_sync<EncodingProperties>(
      logPrefix, &IEncodingComponent::getEncodingProperties);
}

SpecificEncodingProperties
EncodingComponentWrapper::getEncodingPropertiesForParameters(
    const EncodingParameters &params) {
  TRACE_METHOD(params);
  return post_sync<SpecificEncodingProperties>(
      logPrefix, &IEncodingComponent::getEncodingPropertiesForParameters,
      params);
}
...
void EncodingComponentWrapper::encodeBytes(CMTypes::EncodingHandle handle,
  ...
  post(logPrefix, &IEncodingComponent::encodeBytes, handle.handle, params,
       bytes);
}
...
void EncodingComponentWrapper::decodeBytes(CMTypes::DecodingHandle handle,
                                           ...
                                           const std::vector<uint8_t> &bytes) {
  TRACE_METHOD(handle, params, bytes.size());
  post(logPrefix, &IEncodingComponent::decodeBytes, handle.handle, params,
       bytes);
}
} // namespace Raceboat
