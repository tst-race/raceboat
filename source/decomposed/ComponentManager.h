...
public:
  ...
  virtual CMTypes::CmInternalStatus
  destroyLink(CMTypes::ComponentWrapperHandle postId,
              CMTypes::LinkSdkHandle handle, const LinkID &linkId);

  virtual CMTypes::CmInternalStatus
  createLink(CMTypes::ComponentWrapperHandle postId,
  ...
  virtual CMTypes::CmInternalStatus createLinkFromAddress(
  ...
  virtual CMTypes::CmInternalStatus
  onUserInputReceived(CMTypes::ComponentWrapperHandle postId,
                      CMTypes::UserSdkHandle handle, bool answered,
                      const std::string &response);

  virtual CMTypes::CmInternalStatus
  onUserAcknowledgementReceived(CMTypes::ComponentWrapperHandle postId,
                                CMTypes::UserSdkHandle handle);

  // Common Apis
  virtual CMTypes::CmInternalStatus
  requestPluginUserInput(CMTypes::ComponentWrapperHandle postId,
                         const std::string &componentId, const std::string &key,
                         const std::string &prompt, bool cache);
  virtual CMTypes::CmInternalStatus
  requestCommonUserInput(CMTypes::ComponentWrapperHandle postId,
                         const std::string &componentId,
                         const std::string &key);
  virtual CMTypes::CmInternalStatus
  updateState(CMTypes::ComponentWrapperHandle postId,
              const std::string &componentId, ComponentState state);

  // IEncodingSdk APIs
  virtual CMTypes::CmInternalStatus
  onBytesEncoded(CMTypes::ComponentWrapperHandle postId,
                 CMTypes::EncodingHandle handle, std::vector<uint8_t> &&bytes,
  ...
  virtual CMTypes::CmInternalStatus
  onPackageStatusChanged(CMTypes::ComponentWrapperHandle postId,
                         CMTypes::PackageFragmentHandle handle,
                         PackageStatus status);

  virtual CMTypes::CmInternalStatus
  onEvent(CMTypes::ComponentWrapperHandle postId, const Event &event);
  ...
  virtual std::vector<CMTypes::PackageFragmentHandle>
  getPackageHandlesForAction(CMTypes::ActionInfo *info);
  virtual void actionDone(CMTypes::ActionInfo *info);

  // Testing
  std::string toString() const;
  CMTypes::CmInternalStatus
  waitForCallbacks(CMTypes::ComponentWrapperHandle postId);
...
public:
  ...
  std::unordered_map<CMTypes::UserSdkHandle,
                     std::pair<CMTypes::UserComponentHandle, std::string>>
      userInputMap;

  // This should be locked while in any CMInternal function
  // TODO: currently, the action thread in action manager does stuff with member
  // variables. If instead it posted to the CMInternal thread, this mutex
...
public:
  ...
  virtual PluginResponse sendPackage(RaceHandle handle,
                                     ConnectionID connectionId, EncPkg pkg,
                                     double timeoutTimestamp,
                                     uint64_t batchId) override;

  virtual PluginResponse openConnection(RaceHandle handle, LinkType linkType,
                                        LinkID linkId, std::string linkHints,
                                        int32_t sendTimeout) override;

  virtual PluginResponse closeConnection(RaceHandle handle,
  ...
  virtual PluginResponse loadLinkAddress(RaceHandle handle,
                                         std::string channelGid,
                                         std::string linkAddress) override;

  virtual PluginResponse
  loadLinkAddresses(RaceHandle handle, std::string channelGid,
                    std::vector<std::string> linkAddresses) override;

  virtual PluginResponse
  createLinkFromAddress(RaceHandle handle, std::string channelGid,
                        std::string linkAddress) override;
  ...
  virtual PluginResponse
  onUserInputReceived(RaceHandle handle, bool answered,
                      const std::string &response) override;

  virtual PluginResponse
  onUserAcknowledgementReceived(RaceHandle handle) override;

  virtual PluginResponse serveFiles(LinkID /*linkId*/,
                                    std::string /*path*/) override {
    return PLUGIN_ERROR;
  };

  virtual PluginResponse
  createBootstrapLink(RaceHandle /*handle*/, std::string /*channelGid*/,
                      std::string /*passphrase*/) override {
    return PLUGIN_ERROR;
  };

  virtual PluginResponse flushChannel(RaceHandle /* handle */,
                                      std::string /* channelGid */,
                                      uint64_t /* batchId */) override {
    return PLUGIN_OK;
  };
  ...
  // Common Apis
  virtual ChannelResponse requestPluginUserInput(std::string componentId,
                                                 const std::string &key,
                                                 const std::string &prompt,
                                                 bool cache);
  virtual ChannelResponse requestCommonUserInput(std::string componentId,
                                                 const std::string &key);
  virtual ChannelResponse updateState(std::string componentId,
...
protected:
  ...
  ChannelResponse post(const std::string &logPrefix, T &&function,
                       Args &&... args);

  template <typename T, typename... Args>
  PluginResponse post_sync(const std::string &logPrefix, T &&function,
                           Args &&... args);
