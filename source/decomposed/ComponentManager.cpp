...
ComponentManager::ComponentManager(
    IRaceSdkComms &sdk, const Composition &composition,
    ...
    : sdk(sdk), handler("component-mananger-thread", 1 << 20, 1 << 20),
      manager(this, sdk, composition, transportPlugin, usermodelPlugin,
              encodingPlugins) {
...
ChannelResponse ComponentManager::post(const std::string &logPrefix,
  ...
  try {
    auto workFunc = [logPrefix, postHandle, postId, function,
      ...
      if (status != CMTypes::OK) {
        helper::logError(logPrefix + "Returned " +
                         cmInternalStatusToString(status) +
                         ", postId: " + postId);
...
template <typename T, typename... Args>
PluginResponse ComponentManager::post_sync(const std::string &logPrefix,
                                           T &&function, Args &&... args) {
  ComponentWrapperHandle postHandle{nextPostId++};
  std::string postId = std::to_string(postHandle.handle);
  helper::logDebug(logPrefix + "Posting postId: " + postId);
...
PluginResponse ComponentManager::sendPackage(RaceHandle handle,
                                             ConnectionID connectionId,
                                             EncPkg pkg,
                                             double timeoutTimestamp,
                                             uint64_t batchId) {
  TRACE_METHOD();
  auto resp = post_sync(logPrefix, &ComponentManagerInternal::sendPackage,
                        PackageSdkHandle{handle}, connectionId, std::move(pkg),
                        timeoutTimestamp, batchId);
  return resp;
}

PluginResponse ComponentManager::openConnection(RaceHandle handle,
                                                LinkType linkType,
                                                LinkID linkId,
  ...
  auto resp = post(logPrefix, &ComponentManagerInternal::openConnection,
                   ConnectionSdkHandle{handle}, linkType, linkId, linkHints,
                   sendTimeout);
  return (resp.status == CM_OK) ? PLUGIN_OK : PLUGIN_ERROR;
}

PluginResponse ComponentManager::closeConnection(RaceHandle handle,
                                                 ConnectionID connectionId) {
  TRACE_METHOD();
...
PluginResponse ComponentManager::destroyLink(RaceHandle handle, LinkID linkId) {
  TRACE_METHOD();
  auto resp = post(logPrefix, &ComponentManagerInternal::destroyLink,
                   LinkSdkHandle{handle}, linkId);
  return (resp.status == CM_OK) ? PLUGIN_OK : PLUGIN_ERROR;
}

PluginResponse ComponentManager::createLink(RaceHandle handle,
                                            std::string channelGid) {
  TRACE_METHOD();
  auto resp = post(logPrefix, &ComponentManagerInternal::createLink,
                   LinkSdkHandle{handle}, channelGid);
  return (resp.status == CM_OK) ? PLUGIN_OK : PLUGIN_ERROR;
}

PluginResponse ComponentManager::loadLinkAddress(RaceHandle handle,
                                                 std::string channelGid,
                                                 std::string linkAddress) {
  TRACE_METHOD();
  auto resp = post(logPrefix, &ComponentManagerInternal::loadLinkAddress,
                   LinkSdkHandle{handle}, channelGid, linkAddress);
  return (resp.status == CM_OK) ? PLUGIN_OK : PLUGIN_ERROR;
}

PluginResponse
ComponentManager::loadLinkAddresses(RaceHandle handle, std::string channelGid,
                                    std::vector<std::string> linkAddresses) {
  TRACE_METHOD();
  auto resp = post(logPrefix, &ComponentManagerInternal::loadLinkAddresses,
                   LinkSdkHandle{handle}, channelGid, linkAddresses);
  return (resp.status == CM_OK) ? PLUGIN_OK : PLUGIN_ERROR;
}

PluginResponse ComponentManager::createLinkFromAddress(
    RaceHandle handle, std::string channelGid, std::string linkAddress) {
  TRACE_METHOD();
...
PluginResponse ComponentManager::deactivateChannel(RaceHandle handle,
                                                   std::string channelGid) {
  TRACE_METHOD();
...
PluginResponse ComponentManager::activateChannel(RaceHandle handle,
                                                 std::string channelGid,
                                                 std::string roleName) {
  TRACE_METHOD();
...
PluginResponse
ComponentManager::onUserInputReceived(RaceHandle handle, bool answered,
...
PluginResponse
ComponentManager::onUserAcknowledgementReceived(RaceHandle handle) {
  TRACE_METHOD();
  auto resp =
      post(logPrefix, &ComponentManagerInternal::onUserAcknowledgementReceived,
           UserSdkHandle{handle});
  return (resp.status == CM_OK) ? PLUGIN_OK : PLUGIN_ERROR;
}

// Common Apis
ChannelResponse ComponentManager::requestPluginUserInput(
    std::string componentId, const std::string &key, const std::string &prompt,
    bool cache) {
  TRACE_METHOD();
  return post(logPrefix, &ComponentManagerInternal::requestPluginUserInput,
              componentId, key, prompt, cache);
}
ChannelResponse
ComponentManager::requestCommonUserInput(std::string componentId,
                                         const std::string &key) {
  TRACE_METHOD();
  return post(logPrefix, &ComponentManagerInternal::requestCommonUserInput,
              componentId, key);
}
ChannelResponse ComponentManager::updateState(std::string componentId,
                                              ComponentState state) {
  TRACE_METHOD();
  return post(logPrefix, &ComponentManagerInternal::updateState, componentId,
              state);
}

// IEncodingSdk APIs
ChannelResponse
ComponentManager::onBytesEncoded(RaceHandle handle,
                                 const std::vector<uint8_t> &bytes,
                                 EncodingStatus status) {
  TRACE_METHOD(handle, bytes.size(), status);
  return post(logPrefix, &ComponentManagerInternal::onBytesEncoded,
              EncodingHandle{handle}, bytes, status);
}

ChannelResponse
ComponentManager::onBytesDecoded(RaceHandle handle,
                                 const std::vector<uint8_t> &bytes,
                                 EncodingStatus status) {
  TRACE_METHOD(handle, bytes.size(), status);
  return post(logPrefix, &ComponentManagerInternal::onBytesDecoded,
              DecodingHandle{handle}, bytes, status);
}

// ITransportSdk APIs
ChannelResponse
ComponentManager::onLinkStatusChanged(RaceHandle handle, const LinkID &linkId,
                                      LinkStatus status,
                                      const LinkParameters &params) {
  TRACE_METHOD(handle, linkId, status, params);
  return post(logPrefix, &ComponentManagerInternal::onLinkStatusChanged,
              LinkSdkHandle{handle}, linkId, status, params);
}

ChannelResponse ComponentManager::onPackageStatusChanged(RaceHandle handle,
...
ChannelResponse ComponentManager::onReceive(const LinkID &linkId,
                                            const EncodingParameters &params,
                                            const std::vector<uint8_t> &bytes) {
  TRACE_METHOD(linkId, params, bytes.size());
  return post(logPrefix, &ComponentManagerInternal::onReceive, linkId, params,
              bytes);
}
...
ComponentManager::listDir(const std::string &directoryPath) {
  TRACE_METHOD(directoryPath);
  return manager.sdk.listDir(directoryPath);
}

ChannelResponse
ComponentManager::appendFile(const std::string &filepath,
...
ChannelResponse
ComponentManager::writeFile(const std::string &filepath,
                            const std::vector<std::uint8_t> &data) {
  TRACE_METHOD(filepath, data.size());
...
ComponentManager::readFile(const std::string &filepath) {
...
ChannelProperties ComponentManager::getChannelProperties() {
  TRACE_METHOD();
  return manager.sdk.getChannelProperties(
      manager.lifetimeManager.composition.id);
}
...
ComponentManagerInternal::ComponentManagerInternal(
    ComponentManager *manager, IRaceSdkComms &sdk,
    const Composition &composition, IComponentPlugin &transportPlugin,
    IComponentPlugin &usermodelPlugin,
    const std::unordered_map<std::string, IComponentPlugin *> &encodingPlugins)
    : manager(manager), sdk(sdk), actionManager(*this),
      connectionManager(*this),
      lifetimeManager(*this, composition, transportPlugin, usermodelPlugin,
                      encodingPlugins),
      linkManager(*this), packageManager(*this), receiveManager(*this) {
  TRACE_METHOD(composition.description());
}

ComponentManagerInternal::~ComponentManagerInternal() { TRACE_METHOD(); }
...
std::vector<Link *> ComponentManagerInternal::getLinks() {
  ...
  std::transform(linkManager.links.begin(), linkManager.links.end(),
                 links.begin(), [](auto &pair) { return pair.second.get(); });
  return links;
}

Connection *
ComponentManagerInternal::getConnection(const ConnectionID &connId) {
  return connectionManager.connections.at(connId).get();
}

CmInternalStatus
ComponentManagerInternal::init(ComponentWrapperHandle postId,
...
PluginResponse
ComponentManagerInternal::shutdown(ComponentWrapperHandle postId) {
  TRACE_METHOD(postId);
  lifetimeManager.shutdown(postId);
  return PLUGIN_OK;
}

PluginResponse ComponentManagerInternal::sendPackage(
    ComponentWrapperHandle postId, PackageSdkHandle handle,
    const ConnectionID &connId, EncPkg &&pkg, double timeoutTimestamp,
    uint64_t batchId) {
  TRACE_METHOD(postId, handle, connId, pkg.getSize(), timeoutTimestamp,
               batchId);
  ...
  PluginResponse response = packageManager.sendPackage(
      postId, now, handle, connId, std::move(pkg), timeoutTimestamp, batchId);

  return response;
}

CmInternalStatus ComponentManagerInternal::openConnection(
  ...
  return connectionManager.openConnection(postId, handle, linkType, linkId,
                                          linkHints, sendTimeout);
}

CmInternalStatus
ComponentManagerInternal::closeConnection(ComponentWrapperHandle postId,
...
CmInternalStatus
ComponentManagerInternal::createLink(ComponentWrapperHandle postId,
...
CmInternalStatus ComponentManagerInternal::loadLinkAddress(
    ComponentWrapperHandle postId, LinkSdkHandle handle,
    const std::string &inChannelGid, const std::string &linkAddress) {
...
CmInternalStatus ComponentManagerInternal::loadLinkAddresses(
  ...
  EXPECT_STATE(ACTIVATED, ERROR);
  EXPECT_CHANNEL_MATCHES(inChannelGid, ERROR);
  return linkManager.loadLinkAddresses(postId, handle, inChannelGid,
                                       linkAddresses);
}

CmInternalStatus ComponentManagerInternal::createLinkFromAddress(
    ComponentWrapperHandle postId, LinkSdkHandle handle,
    const std::string &inChannelGid, const std::string &linkAddress) {
  TRACE_METHOD(postId, handle, inChannelGid, linkAddress);
  std::unique_lock<std::recursive_mutex> lock(dataMutex);
  EXPECT_STATE(ACTIVATED, ERROR);
  EXPECT_CHANNEL_MATCHES(inChannelGid, ERROR);
  return linkManager.createLinkFromAddress(postId, handle, inChannelGid,
                                           linkAddress);
}

CmInternalStatus ComponentManagerInternal::destroyLink(
    ComponentWrapperHandle postId, LinkSdkHandle handle, const LinkID &linkId) {
...
CmInternalStatus
ComponentManagerInternal::deactivateChannel(ComponentWrapperHandle postId,
...
CmInternalStatus ComponentManagerInternal::activateChannel(
    ComponentWrapperHandle postId, ChannelSdkHandle handle,
    const std::string &inChannelGid, const std::string &roleName) {
  TRACE_METHOD(postId, handle, inChannelGid, roleName);
  std::unique_lock<std::recursive_mutex> lock(dataMutex);
  EXPECT_STATE(UNACTIVATED, ERROR);
  EXPECT_CHANNEL_MATCHES(inChannelGid, ERROR);
  return lifetimeManager.activateChannel(postId, handle, inChannelGid,
                                         roleName);
}

CmInternalStatus ComponentManagerInternal::onUserInputReceived(
    ComponentWrapperHandle postId, UserSdkHandle handle, bool answered,
    const std::string &response) {
  TRACE_METHOD(postId, handle, answered, response);
  std::unique_lock<std::recursive_mutex> lock(dataMutex);
  EXPECT_STATE((CREATING_COMPONENTS | WAITING_FOR_COMPONENTS | ACTIVATED),
               ERROR);
  ...
  if (it != userInputMap.end()) {
    ...
    try {
      ...
      helper::logError(logPrefix +
                       "Could not find component with id: " + componentId);
      return ERROR;
    }
  }

  helper::logError(logPrefix +
                   "No mapping found for handle: " + handle.to_string());
  return ERROR;
}
...
CmInternalStatus ComponentManagerInternal::requestPluginUserInput(
    ComponentWrapperHandle postId, const std::string &componentId,
  ...
  userInputMap[{sdkResp.handle}] = {UserComponentHandle{postId.handle},
                                    componentId};
  return OK;
}

CmInternalStatus
ComponentManagerInternal::requestCommonUserInput(ComponentWrapperHandle postId,
...
CmInternalStatus
ComponentManagerInternal::updateState(ComponentWrapperHandle postId,
                                      const std::string &componentId,
...
CmInternalStatus ComponentManagerInternal::onBytesEncoded(
    ComponentWrapperHandle postId, EncodingHandle handle,
    std::vector<uint8_t> &&bytes, EncodingStatus status) {
  TRACE_METHOD(postId, handle, bytes.size(), status);
  std::unique_lock<std::recursive_mutex> lock(dataMutex);
  EXPECT_STATE(ACTIVATED, ERROR);
  return packageManager.onBytesEncoded(postId, handle, std::move(bytes),
                                       status);
}

CmInternalStatus ComponentManagerInternal::onBytesDecoded(
    ComponentWrapperHandle postId, DecodingHandle handle,
    std::vector<uint8_t> &&bytes, EncodingStatus status) {
  TRACE_METHOD(postId, handle, bytes.size(), status);
  std::unique_lock<std::recursive_mutex> lock(dataMutex);
  EXPECT_STATE(ACTIVATED, ERROR);
  return receiveManager.onBytesDecoded(postId, handle, std::move(bytes),
                                       status);
}

CmInternalStatus ComponentManagerInternal::onLinkStatusChanged(
    ComponentWrapperHandle postId, LinkSdkHandle handle, const LinkID &linkId,
  ...
  return linkManager.onLinkStatusChanged(postId, handle, linkId, status,
                                         params);
}

CmInternalStatus
ComponentManagerInternal::onPackageStatusChanged(ComponentWrapperHandle postId,
...
CmInternalStatus
ComponentManagerInternal::onEvent(ComponentWrapperHandle postId,
                                  const Event &event) {
  TRACE_METHOD(postId);
  std::unique_lock<std::recursive_mutex> lock(dataMutex);
  EXPECT_STATE(ACTIVATED, ERROR);
...
CmInternalStatus ComponentManagerInternal::onReceive(
...
CmInternalStatus
ComponentManagerInternal::onTimelineUpdated(ComponentWrapperHandle postId) {
  TRACE_METHOD(postId);
  std::unique_lock<std::recursive_mutex> lock(dataMutex);
  EXPECT_STATE(ACTIVATED, ERROR);
...
CmInternalStatus
ComponentManagerInternal::markFailed(ComponentWrapperHandle postId) {
  TRACE_METHOD(postId);
  std::unique_lock<std::recursive_mutex> lock(dataMutex);
...
std::vector<PackageFragmentHandle>
ComponentManagerInternal::getPackageHandlesForAction(ActionInfo *info) {
...
CmInternalStatus
ComponentManagerInternal::waitForCallbacks(ComponentWrapperHandle postId) {
  TRACE_METHOD(postId);
...
void tuplePrint(std::ostream &out, const std::tuple<A...> &tup,
...
template <class C> std::string listString(const C &container) {
  std::stringstream ss;
  ss << "[";
...
template <class C> std::string mapString(const C &container) {
...
std::string
mapString(const std::unordered_map<K, std::unique_ptr<V>> &container) {
  // use map for ordering
...
std::ostream &operator<<(std::ostream &out,
                         const std::shared_ptr<IComponentBase> &component) {
...
std::ostream &operator<<(std::ostream &out,
                         const ComponentManagerInternal &manager) {
