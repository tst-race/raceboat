...
operator<<(std::ostream &out,
           const std::vector<std::unique_ptr<CMTypes::PackageFragmentInfo>>
               &fragments) {
  out << "[";
...
std::string packageFragmentInfoToString(const PackageFragmentInfo &fragment) {
  return "PackageFragmentInfo{package: " +
         std::to_string(fragment.package->sdkHandle.handle) + ", action: " +
         (fragment.action ? std::to_string(fragment.action->action.actionId)
                          : "nullptr") +
...
std::string packageListToString(
    const std::vector<PackageFragmentInfo *> &packageFragments) {
  std::stringstream ss;
  ss << "[";
...
static std::ostream &operator<<(
    std::ostream &out,
    const std::deque<std::unique_ptr<CMTypes::PackageInfo>> &packageQueue) {
  out << "[";
...
std::ostream &operator<<(std::ostream &out, const Link &link) {
  // for ordering
  std::set<ConnectionID> conns{link.connections.begin(),
...
std::ostream &operator<<(std::ostream &out, const Connection &conn) {
  return out << "Connection{ connId " << conn.connId << ", linkId "
             << conn.linkId << " }";
}
