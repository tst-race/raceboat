...
ComponentLifetimeManager::ComponentLifetimeManager(
    ...
    : state(CMTypes::State::INITIALIZING), manager(manager),
      composition(composition), transportPlugin(transportPlugin),
      usermodelPlugin(usermodelPlugin),
      encodingPlugins(encodingPlugins), activateHandle{NULL_RACE_HANDLE} {}

ComponentLifetimeManager::~ComponentLifetimeManager() {
  TRACE_METHOD();
  if (transport || usermodel || encodings.size() > 0) {
    helper::logError(logPrefix +
                     "Destroyed but teardown was not called first, aborting");
    std::terminate();
  }
}

CMTypes::CmInternalStatus
ComponentLifetimeManager::init(ComponentWrapperHandle postId,
...
CMTypes::CmInternalStatus
ComponentLifetimeManager::shutdown(ComponentWrapperHandle postId) {
  TRACE_METHOD(postId);
  if (transport || usermodel || encodings.size() > 0) {
...
CMTypes::CmInternalStatus
ComponentLifetimeManager::deactivateChannel(ComponentWrapperHandle postId,
...
CMTypes::CmInternalStatus ComponentLifetimeManager::activateChannel(
  ...
  auto transportSdkWrapper = std::make_unique<TransportSdkWrapper>(
      *manager.manager, composition.transport);
  auto transportComponent = transportPlugin.createTransport(
      composition.transport, transportSdkWrapper.get(), roleName,
      manager.pluginConfig);
  if (transportComponent == nullptr) {
    helper::logError(logPrefix + "Failed to create transport '" +
                     composition.transport + "'");
    return FATAL;
  }
  transport.emplace(composition.id, composition.transport,
                    std::move(transportComponent), manager.manager);
  ...
  auto usermodelSdkWrapper = std::make_unique<UserModelSdkWrapper>(
      *manager.manager, composition.usermodel);
  auto usermodelComponent = usermodelPlugin.createUserModel(
      composition.usermodel, usermodelSdkWrapper.get(), roleName,
      manager.pluginConfig);
  if (usermodelComponent == nullptr) {
    helper::logError(logPrefix + "Failed to create usermodel '" +
                     composition.usermodel + "'");
    return FATAL;
  }
  usermodel.emplace(composition.id, composition.usermodel,
                    std::move(usermodelComponent), manager.manager);
  ...
  for (auto encodingPlugin : encodingPlugins) {
    ...
    auto encodingSdkWrapper =
        std::make_unique<EncodingSdkWrapper>(*manager.manager, name);
    ...
    auto wrapperTup = std::make_tuple(
        composition.id, name, std::move(encodingComponent), manager.manager);

    auto [it, inserted] = encodings.emplace(std::piecewise_construct, nameTup,
                                            std::move(wrapperTup));
...
CMTypes::CmInternalStatus
ComponentLifetimeManager::updateState(ComponentWrapperHandle postId,
...
void ComponentLifetimeManager::fail(ComponentWrapperHandle postId) {
  ...
  manager.sdk.onChannelStatusChanged(activateHandle.handle, composition.id,
                                     CHANNEL_FAILED, {}, RACE_BLOCKING);
}
...
void ComponentLifetimeManager::checkActivated() {
  ...
  bool all_started = std::all_of(
      componentStates.begin(), componentStates.end(), [](auto componentState) {
        return componentState.second == COMPONENT_STATE_STARTED;
      });
  ...
  if (all_started) {
    ...
    manager.setup();
    manager.sdk.onChannelStatusChanged(activateHandle.handle, composition.id,
                                       CHANNEL_AVAILABLE, manager.channelProps,
                                       RACE_BLOCKING);
  }
}
...
std::ostream &operator<<(
    std::ostream &out,
    const std::vector<std::pair<EncodingType, EncodingComponentWrapper *>>
        &encodingsByType) {
  out << "{";
...
std::ostream &
operator<<(std::ostream &out,
           const std::unordered_map<std::string, EncodingComponentWrapper>
               &encodings) {
  out << "{";
...
std::ostream &operator<<(std::ostream &out,
  ...
  return out << "LifetimeManager{"
             ...
             << ", encodings: " << manager.encodings
             << ", wrappers: " << manager.wrappers
