...
ComponentSdkBaseWrapper::ComponentSdkBaseWrapper(ComponentManager &manager,
                                                 const std::string &id)
...
ChannelResponse ComponentSdkBaseWrapper::requestPluginUserInput(
    const std::string &key, const std::string &prompt, bool cache) {
  return manager.requestPluginUserInput(id, key, prompt, cache);
}

ChannelResponse
ComponentSdkBaseWrapper::requestCommonUserInput(const std::string &key) {
...
TransportSdkWrapper::TransportSdkWrapper(ComponentManager &sdk,
                                         const std::string &id)
    : ComponentSdkBaseWrapper(sdk, id) {}

ChannelResponse
ComponentSdkBaseWrapper::makeDir(const std::string &directoryPath) {
  return manager.makeDir(directoryPath);
}

ChannelResponse
ComponentSdkBaseWrapper::removeDir(const std::string &directoryPath) {
  return manager.removeDir(directoryPath);
}

std::vector<std::string>
ComponentSdkBaseWrapper::listDir(const std::string &directoryPath) {
  return manager.listDir(directoryPath);
}

std::vector<std::uint8_t>
ComponentSdkBaseWrapper::readFile(const std::string &filepath) {
  return manager.readFile(filepath);
}

ChannelResponse
ComponentSdkBaseWrapper::appendFile(const std::string &filepath,
                                    const std::vector<std::uint8_t> &data) {
  return manager.appendFile(filepath, data);
}

ChannelResponse
ComponentSdkBaseWrapper::writeFile(const std::string &filepath,
                                   const std::vector<std::uint8_t> &data) {
...
ChannelResponse TransportSdkWrapper::onLinkStatusChanged(
...
ChannelResponse
TransportSdkWrapper::onPackageStatusChanged(RaceHandle handle,
...
ChannelResponse
TransportSdkWrapper::onReceive(const LinkID &linkId,
                               const EncodingParameters &params,
                               const std::vector<uint8_t> &bytes) {
  return manager.onReceive(linkId, params, bytes);
}

UserModelSdkWrapper::UserModelSdkWrapper(ComponentManager &sdk,
                                         const std::string &id)
    : ComponentSdkBaseWrapper(sdk, id) {}
...
EncodingSdkWrapper::EncodingSdkWrapper(ComponentManager &sdk,
                                       const std::string &id)
    : ComponentSdkBaseWrapper(sdk, id) {}

ChannelResponse
EncodingSdkWrapper::onBytesEncoded(RaceHandle handle,
                                   const std::vector<uint8_t> &bytes,
...
ChannelResponse
EncodingSdkWrapper::onBytesDecoded(RaceHandle handle,
                                   const std::vector<uint8_t> &bytes,
                                   EncodingStatus status) {
