...
// powers of 2 so they can by or'd together for checking if current state
// matches
enum State {
  INITIALIZING = 1 << 0,
  UNACTIVATED = 1 << 1,
...
template <int> struct HandleStruct {
  RaceHandle handle;
  ...
  std::string to_string() const { return std::to_string(handle); }
};
...
std::ostream &operator<<(std::ostream &out,
                         const HandleStruct<N> &handleStruct) {
 ...
 * EncodingMode
 *
 * SINGLE indicates a package is not fragmented across multiple actions or batch
 * so that multiple packages fit in a single action
 *
 * BATCH does not fragment across multiple packages but does allow
 * multiple packages to fit in a single action. The bytes encoded are of the
 * form (<len><package>)*.
 *
 * FRAGMENT_SINGLE_PRODUCER does fragment across a multiple packages and allows
 * batching within a single package, but only allows receiving packages from a
 * single producer. This is because there is no way to identify the sender and
 * so fragment may get intermingled. The bytes encoded are of the form <fragment
 * id><flags>(<len><package>)*.
 *
 * FRAGMENT_MULTIPLE_PRODUCER is like the single
 * producer version but also includes a producer identifier allowing it avoid
 * intermingling fragments from different producers. The bytes encoded are of
 * the form <producer id><fragment id><flags>(<len><package>)*.
 */
...
// This struct is owned by PackageInfo and will live until the package is
// removed
struct PackageFragmentInfo {
  ...
  // This is managed by the ComponentPackage manager and will point to the
  // action used to send this fragment if it exists. The action may have been
  // completed and removed, in which case this pointer will be null.
  ActionInfo *action;

  // offset and len refer to the offset and len of the bytes of the package in
  // this fragment
  size_t offset;
  size_t len;
...
std::string
packageFragmentInfoToString(const PackageFragmentInfo &packageFragmentInfo);
std::ostream &operator<<(std::ostream &out,
                         const PackageFragmentInfo &packageFragmentInfo);

// This struct is owned by the ComponentActionManager and lives in its global
...
struct ActionInfo {
  ...
  // The PackageFragmentInfo lives inside packageInfo which lives on the link's
  // packageQueue and outlives this struct because ComponentPackageManager's
  // onPackageStatusChanged removes the package and is only processed after the
  // ComponentActionManager has executed and removed the action.
...
struct ActionCompare {
  bool operator()(const Action &a, const Action &b) const {
    return a.timestamp < b.timestamp ||
           (a.timestamp == b.timestamp && a.actionId < b.actionId);
  }
  bool operator()(const Action *a, const Action *b) const {
    return (*this)(*a, *b);
  }
...
// This struct is owned by the ComponentPackageManager and lives on a link's
// packageQueue.
struct PackageInfo {
...
struct Link {
  ...
  // The ActionInfo is managed by the ComponentActionManager, which will add &
  // remove them from this queue. These are pointers to the actions as they live
  // in the action manager's global action timeline/queue.
  std::deque<ActionInfo *> actionQueue;
  // The PackageInfo is managed by the ComponentPackageManager, which will add &
  // remove them from this queue. The packages live here in the link's package
  // queue. unique_ptr is used to keep the address the same, even if the item in
  // the deque is moved.
...
struct Connection {
  Connection(const ConnectionID &connId, const LinkID &linkId)
      : connId(connId), linkId(linkId) {}
  ConnectionID connId;
  LinkID linkId;
};
...
namespace std {
template <int N> struct hash<Raceboat::CMTypes::HandleStruct<N>> {
  std::size_t
  operator()(const Raceboat::CMTypes::HandleStruct<N> &handleStruct) const {
