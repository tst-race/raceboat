...
public:
  virtual void onUserInputReceived(CMTypes::UserComponentHandle handle,
                                   bool answered,
...
protected:
  ...
  ReturnType post_sync(const std::string &logPrefix, T &&function,
                       Args &&... args);
...
class TransportComponentWrapper
    : public TemplatedComponentWrapper<ITransportComponent> {
public:
  explicit TransportComponentWrapper(
  ...
  virtual void loadLinkAddress(CMTypes::LinkSdkHandle handle,
                               const LinkID &linkId,
                               const std::string &linkAddress);
  virtual void loadLinkAddresses(CMTypes::LinkSdkHandle handle,
                                 const LinkID &linkId,
                                 const std::vector<std::string> &linkAddress);
  virtual void createLinkFromAddress(CMTypes::LinkSdkHandle handle,
                                     const LinkID &linkId,
                                     const std::string &linkAddress);
  ...
  virtual void enqueueContent(const EncodingParameters &params,
                              const Action &action,
                              const std::vector<uint8_t> &content);
  virtual void dequeueContent(const Action &action);

  virtual void
  doAction(const std::vector<CMTypes::PackageFragmentHandle> &handles,
           const Action &action);
};

class UserModelComponentWrapper
    : public TemplatedComponentWrapper<IUserModelComponent> {
public:
  explicit UserModelComponentWrapper(
      const std::string &channelName, const std::string &componentName,
...
class EncodingComponentWrapper
    : public TemplatedComponentWrapper<IEncodingComponent> {
public:
  explicit EncodingComponentWrapper(
      const std::string &channelName, const std::string &componentName,
  ...
  getEncodingPropertiesForParameters(const EncodingParameters &params);

  virtual void encodeBytes(CMTypes::EncodingHandle handle,
                           const EncodingParameters &params,
                           const std::vector<uint8_t> &bytes);

  virtual void decodeBytes(CMTypes::DecodingHandle handle,
                           const EncodingParameters &params,
                           const std::vector<uint8_t> &bytes);
};
