...
class Core {
public:
    // Unintialized constructor. This won't be functional, but is useful for testing.
    Core();

    // The main constructor
    Core(std::string race_dir, ChannelParamStore params);

    // Explicitly delete these. The plugins assume that the address of Core does not change as they
    // maintain references to it that must remain valid.
    ...
    virtual SdkResponse updateLinkProperties(PluginContainer &plugin, const LinkID &linkId,
                                             const LinkProperties &properties);
    virtual SdkResponse receiveEncPkg(PluginContainer &plugin, const EncPkg &pkg,
                                      const std::vector<ConnectionID> &connIDs);
    virtual ConnectionID generateConnectionId(PluginContainer &plugin, const LinkID &linkId);
    virtual LinkID generateLinkId(PluginContainer &plugin, const ChannelId &channelGid);
