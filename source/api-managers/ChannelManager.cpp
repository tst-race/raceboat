...
ChannelProperties ChannelManager::getChannelProperties(const ChannelId &channelId) {
...
void ChannelManager::onChannelStatusChanged(RaceHandle /* handle */, const ChannelId &channelGid,
...
ActivateChannelStatusCode ChannelManager::activateChannel(RaceHandle handle, std::string channelGid,
                                                          std::string roleName) {
    try {
        std::unique_lock propLock(propsMutex);
        auto &props = channelProps.at(channelGid);
        if (props.channelStatus != CHANNEL_ENABLED && props.channelStatus != CHANNEL_STARTING &&
