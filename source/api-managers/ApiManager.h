...
// ApiManager drives several state engines (connection, send, recv, send-reply, listen, dial,
// pre-conn-obj, conn-obj). A conn state machine context is created by the startConnStateMachine()
// call which gets called by the other state machines. Pre-conn-obj is created by a listen state
// machine after receiving a message from a client. It turns into a conn-obj state machine after
// accept is called. The other state machines are created directely by calls from the app.
...
public:
    ...
    // Library Api calls
    virtual void send(uint64_t postId, SendOptions sendOptions, std::vector<uint8_t> data,
                      std::function<void(ApiStatus)> callback);
    virtual void sendReceive(uint64_t postId, SendOptions sendOptions, std::vector<uint8_t> data,
                             std::function<void(ApiStatus, std::vector<uint8_t>)> callback);
    virtual void dial(uint64_t postId, SendOptions sendOptions, std::vector<uint8_t> data,
                      std::function<void(ApiStatus, RaceHandle)> callback);
    virtual void getReceiveObject(uint64_t postId, ReceiveOptions recvOptions,
                                  std::function<void(ApiStatus, LinkAddress, RaceHandle)> callback);
    virtual void receive(uint64_t postId, OpHandle handle,
                         std::function<void(ApiStatus, std::vector<uint8_t>)> callback);
    ...
    virtual void listen(uint64_t postId, ReceiveOptions recvOptions,
                        std::function<void(ApiStatus, LinkAddress, RaceHandle)> callback);
    virtual void accept(uint64_t postId, OpHandle handle,
                        std::function<void(ApiStatus, RaceHandle)> callback);

    virtual void read(uint64_t postId, OpHandle handle,
                      std::function<void(ApiStatus, std::vector<uint8_t>)> callback);
    virtual void write(uint64_t postId, OpHandle handle, std::vector<uint8_t> bytes,
                       std::function<void(ApiStatus)> callback);
    virtual void close(uint64_t postId, OpHandle handle, std::function<void(ApiStatus)> callback);

    // state machine callbacks
    virtual ActivateChannelStatusCode activateChannel(ApiContext &context, RaceHandle handle,
    ...
    virtual void connStateMachineConnected(RaceHandle contextHandle, ConnectionID connId,
                                           std::string linkAddress);

    virtual void onStateMachineFailed(uint64_t postId, RaceHandle contextHandle);
    virtual void onStateMachineFinished(uint64_t postId, RaceHandle contextHandle);
    virtual void onConnStateMachineConnected(uint64_t postId, RaceHandle contextHandle,
                                             ConnectionID connId, std::string linkAddress);

    virtual void onChannelStatusChangedForContext(uint64_t postId, RaceHandle contextHandle,
    ...
    virtual void onChannelStatusChanged(uint64_t postId, RaceHandle handle,
                                        const ChannelId &channelGid, ChannelStatus status,
                                        const ChannelProperties &properties);
    virtual void onLinkStatusChanged(uint64_t postId, RaceHandle handle, const LinkID &linkId,
    ...
    virtual void onPackageStatusChanged(uint64_t postId, RaceHandle handle, PackageStatus status);

    // connection state triggers
    virtual RaceHandle startConnStateMachine(RaceHandle contextHandle, ChannelId channelId,
    ...
    virtual bool onListenAccept(RaceHandle contextHandle,
                                std::function<void(ApiStatus, RaceHandle)> acceptCb);
    virtual bool detachConnSM(RaceHandle contextHandle, RaceHandle connSMContextHandle);
    ...
    virtual void registerPackageId(ApiContext &context, const ConnectionID &connId,
                                   const std::string &id);
...
public:
    ...
    std::unordered_map<ChannelId, std::pair<ChannelStatus, ChannelProperties>> activatedChannels;
};
...
public:
    ...
    virtual SdkResponse send(SendOptions sendOptions, std::vector<uint8_t> data,
                             std::function<void(ApiStatus)> callback);
    virtual SdkResponse sendReceive(SendOptions sendOptions, std::vector<uint8_t> data,
                                    std::function<void(ApiStatus, std::vector<uint8_t>)> callback);
    ...
    virtual SdkResponse getReceiveObject(
        ...
        std::function<void(ApiStatus, LinkAddress, RaceHandle)> callback);
    virtual SdkResponse receive(OpHandle handle,
                                std::function<void(ApiStatus, std::vector<uint8_t>)> callback);
    virtual SdkResponse receiveRespond(
        OpHandle handle,
        std::function<void(ApiStatus, std::vector<uint8_t>, LinkAddress)> callback);
    virtual SdkResponse listen(ReceiveOptions recvOptions,
                               std::function<void(ApiStatus, LinkAddress, RaceHandle)> callback);
    virtual SdkResponse accept(OpHandle handle,
                               std::function<void(ApiStatus, RaceHandle)> callback);

    virtual SdkResponse read(OpHandle handle,
                             std::function<void(ApiStatus, std::vector<uint8_t>)> callback);
    virtual SdkResponse write(OpHandle handle, std::vector<uint8_t> bytes,
                              std::function<void(ApiStatus)> callback);
    virtual SdkResponse close(OpHandle handle, std::function<void(ApiStatus)> callback);
    ...
    virtual SdkResponse onConnStateMachineConnected(RaceHandle contextHandle, ConnectionID connId,
                                                    std::string linkAddress);
    virtual SdkResponse onChannelStatusChangedForContext(RaceHandle contextHandle,
                                                         RaceHandle callHandle,
    ...
    // Plugin callbacks
    virtual SdkResponse onChannelStatusChanged(PluginContainer &plugin, RaceHandle handle,
                                               const ChannelId &channelGid, ChannelStatus status,
                                               const ChannelProperties &properties);
    virtual SdkResponse onLinkStatusChanged(PluginContainer &plugin, RaceHandle handle,
                                            const LinkID &linkId, LinkStatus status,
                                            const LinkProperties &properties);
    virtual SdkResponse onConnectionStatusChanged(PluginContainer &plugin, RaceHandle handle,
                                                  const ConnectionID &connId,
                                                  ConnectionStatus status,
                                                  const LinkProperties &properties);
    virtual SdkResponse receiveEncPkg(PluginContainer &plugin, const EncPkg &pkg,
                                      const std::vector<ConnectionID> &connIDs);
    virtual SdkResponse onPackageStatusChanged(PluginContainer &plugin, RaceHandle handle,
