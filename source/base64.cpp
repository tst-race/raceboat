...
constexpr char b64_encode_table[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
std::string base64::encode(const std::vector<uint8_t> &data) {
  ...
  for (std::size_t i = 0; i < size / 3; it += 3, ++i) {
    ...
    b64.push_back(
        b64_encode_table[((it[0] & 0x03) << 4) | ((it[1] & 0xF0) >> 4)]);
    b64.push_back(
        b64_encode_table[((it[1] & 0x0F) << 2) | ((it[2] & 0xC0) >> 6)]);
...
static void decode_block(std::vector<uint8_t> &data, T &it) {
  std::int8_t a, b, c, d;
  ...
  if constexpr (I >= 1) {
    data.push_back(((std::uint8_t(a) & 0x3F) << 2) |
                   ((std::uint8_t(b) & 0x30) >> 4));
  }
  if constexpr (I >= 2) {
    data.push_back(((std::uint8_t(b) & 0x0F) << 4) |
                   ((std::uint8_t(c) & 0x3C) >> 2));
  }
  if constexpr (I >= 3) {
    data.push_back(((std::uint8_t(c) & 0x03) << 6) |
                   ((std::uint8_t(d) & 0x3F) >> 0));
  }
}

std::vector<uint8_t> base64::decode(const std::string &b64) {
  if (b64.size() % 4 != 0) {
