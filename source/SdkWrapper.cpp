...
SdkWrapper::SdkWrapper(PluginContainer &container, Core &core)
...
ChannelProperties SdkWrapper::getChannelProperties(std::string channelGid) {
  ...
  ChannelProperties response =
      core.getChannelManager().getChannelProperties(channelGid);
  return response;
}
...
std::vector<ChannelProperties> SdkWrapper::getAllChannelProperties() {
  auto id = container.id;
  TRACE_METHOD(id);
  std::vector<ChannelProperties> response =
      core.getChannelManager().getAllChannelProperties();
  return response;
}
...
SdkResponse SdkWrapper::requestPluginUserInput(const std::string &key,
  ...
  SdkResponse resp =
      container.plugin->onUserInputReceived(handle, answered, response, 0);
...
SdkResponse SdkWrapper::displayInfoToUser(const std::string &,
...
SdkResponse
SdkWrapper::displayBootstrapInfoToUser(const std::string &,
                                       RaceEnums::UserDisplayType,
...
SdkResponse SdkWrapper::onPackageStatusChanged(RaceHandle handle,
                                               PackageStatus status, int32_t) {
  auto id = container.id;
  TRACE_METHOD(id, handle, status);
  return core.onPackageStatusChanged(container, handle, status);
}

SdkResponse SdkWrapper::onConnectionStatusChanged(RaceHandle handle,
                                                  ConnectionID connId,
  ...
  return core.onConnectionStatusChanged(container, handle, connId, status,
                                        properties);
}

SdkResponse SdkWrapper::onLinkStatusChanged(RaceHandle handle, LinkID linkId,
                                            LinkStatus status,
                                            LinkProperties properties,
                                            int32_t) {
  auto id = container.id;
  TRACE_METHOD(id, handle, linkId, status);
  return core.onLinkStatusChanged(container, handle, linkId, status,
                                  properties);
}

SdkResponse SdkWrapper::onChannelStatusChanged(RaceHandle handle,
  ...
  return core.onChannelStatusChanged(container, handle, channelGid, status,
                                     properties);
}

SdkResponse SdkWrapper::updateLinkProperties(LinkID linkId,
                                             LinkProperties properties,
                                             int32_t) {
  auto id = container.id;
...
SdkResponse SdkWrapper::receiveEncPkg(const EncPkg &pkg,
