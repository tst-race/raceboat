...
/*
 * A subclass of streambuf that allows for writes to a single ostream to write
...
public:
  ...
  void setup_hook(std::ostream &o) { o.rdbuf(this); }

  void set_level(RaceLog::LogLevel level) { levelWrite = level; }
  void set_level_cout(RaceLog::LogLevel level) { levelCout = level; }
  void set_level_file(RaceLog::LogLevel level) { levelFile = level; }
...
static std::ofstream redirectStream;
...
void RaceLog::setLogLevelStdout(LogLevel level) {
  ...
  if (level < LL_NONE) {
    log(level, "RaceLog",
        "log level stdout changed to: " + std::string(logLevelToStr(level)),
        "");
  }
}
void RaceLog::setLogLevelFile(LogLevel level) {
  ...
  if (level < LL_NONE) {
    log(level, "RaceLog",
        "log level file changed to: " + std::string(logLevelToStr(level)), "");
  }
}
...
void RaceLog::log(LogLevel level, const std::string &pluginName,
  ...
  const auto nowMs = std::chrono::duration_cast<std::chrono::microseconds>(
                     ...
                     1000000;
  logStream << std::put_time(std::localtime(&nowFormatted), "%F %T") << '.'
...
#ifdef __ANDROID__
  std::string androidLog =
      logLevelToStr(level) + std::string(" ") + pluginName + ": " + message;
...
void RaceLog::setLogRedirectPath(const std::string &path) {
  if (!path.empty()) {
    // ensure path exists
    ...
    if (!std::filesystem::exists(fspath)) {
      ...
      logError("", "setLogRedirectPath: " + path + " is a directory", "");
      return;
    }

    // redirect ouput to the file
    redirectStream.open(path);
    std::ofstream file(path.c_str());
    logStream.rdbuf(redirectStream.rdbuf());
  } else {
    if (redirectStream.is_open()) {
      redirectStream.close();
    }
    logStream.rdbuf(&logBuffer);
  }
}

void RaceLog::logDebug(const std::string &pluginName,
...
void RaceLog::logWarning(const std::string &pluginName,
                         const std::string &message,
                         const std::string &stackTrace) {
  log(LL_WARNING, pluginName, get_this_thread_id_prefix() + message,
      stackTrace);
}

void RaceLog::logError(const std::string &pluginName,
                       const std::string &message,
                       const std::string &stackTrace) {
