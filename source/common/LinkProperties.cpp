...
LinkProperties::LinkProperties()
    : linkType(LT_UNDEF), transmissionType(TT_UNDEF), connectionType(CT_UNDEF),
      sendType(ST_UNDEF), reliable(false), isFlushable(false), duration_s(-1),
      period_s(-1), mtu(-1) {}
...
std::string linkPropertiesToString(const LinkProperties &props) {
  ...
  ss << " TransmissionType = " +
            transmissionTypeToString(props.transmissionType);
