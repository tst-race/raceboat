...
EncPkg::EncPkg(uint64_t _traceId, uint64_t _spanId, const RawData &_cipherText)
    : traceId(_traceId), spanId(_spanId),
...
EncPkg::EncPkg(RawData rawData)
  ...
  // TODO: should we throw an exception if the incoming size is greater than
  // zero but less than the size TRACE_ID_LENGTH + SPAN_ID_LENGTH?
  if (rawData.size() >=
    ...
    std::memcpy(&packageType,
                ...
                PACKAGE_TYPE_LENGTH);
    cipherText = RawData(rawData.begin() + (TRACE_ID_LENGTH + SPAN_ID_LENGTH +
                                            PACKAGE_TYPE_LENGTH),
                         rawData.end());
  }
}
...
RawData EncPkg::getCipherText() const { return cipherText; }

uint64_t EncPkg::getTraceId() const { return traceId; }

uint64_t EncPkg::getSpanId() const { return spanId; }
...
void EncPkg::setTraceId(uint64_t value) { spanId = value; }

void EncPkg::setSpanId(uint64_t value) { spanId = value; }
...
size_t EncPkg::getSize() const {
  return cipherText.size() + TRACE_ID_LENGTH + SPAN_ID_LENGTH +
         PACKAGE_TYPE_LENGTH;
}
