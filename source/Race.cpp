...
std::pair<ApiStatus, std::vector<uint8_t>> ConnectionObject::read() {
    ...
    // Taking promise by reference is fine because we block waiting for the return. If this function
    // gets changed to timeout, this should be changed as well.
...
ApiStatus ConnectionObject::write(std::vector<uint8_t> bytes) {
    ...
    auto response = core->getApiManager().write(
        handle, std::move(bytes), [&promise](ApiStatus status) { promise.set_value(status); });
    if (response.status != SDK_OK) {
        return ApiStatus::INVALID_ARGUMENT;
    }
...
RespondObject::RespondObject(std::shared_ptr<Core> core, SendOptions sendOptions) :
...
ApiStatus RespondObject::respond(std::vector<uint8_t> data) {
    ...
    std::promise<ApiStatus> promise;
    auto future = promise.get_future();
    core->getApiManager().send(sendOptions, data,
...
ReceiveRespondObject::ReceiveRespondObject(std::shared_ptr<Core> core, OpHandle handle,
                                           ReceiveOptions options) :
    core(core), handle(handle), recvOptions(options) {}

//  blocks until a package is received. This object may be reused after responding.
std::tuple<ApiStatus, std::vector<uint8_t>, RespondObject> ReceiveRespondObject::receive() {
    std::promise<std::tuple<ApiStatus, std::vector<uint8_t>, LinkAddress>> promise;
    auto future = promise.get_future();
    ...
    auto response = core->getApiManager().receiveRespond(
        handle,
        [&promise](ApiStatus status, std::vector<uint8_t> bytes, LinkAddress respondAddress) {
...
std::tuple<ApiStatus, std::string, RespondObject> ReceiveRespondObject::receive_str() {
...
std::pair<ApiStatus, ConnectionObject> AcceptObject::accept() {
    ...
    auto response =
        core->getApiManager().accept(handle, [&promise](ApiStatus status, RaceHandle _handle) {
...
std::tuple<ApiStatus, LinkAddress, ReceiveObject> Race::receive(ReceiveOptions options) {
    TRACE_METHOD();
    ...
    core->getApiManager().getReceiveObject(
        options, [&promise](ApiStatus status, LinkAddress addr, RaceHandle handle) {
...
std::tuple<ApiStatus, LinkAddress, ReceiveRespondObject> Race::receive_respond(
    ReceiveOptions options) {
    std::promise<std::tuple<ApiStatus, LinkAddress, RaceHandle>> promise;
    auto future = promise.get_future();
...
std::tuple<ApiStatus, LinkAddress, AcceptObject> Race::listen(ReceiveOptions options) {
    TRACE_METHOD();
    std::promise<std::tuple<ApiStatus, LinkAddress, RaceHandle>> promise;
    auto future = promise.get_future();

    core->getApiManager().listen(options,
...
ApiStatus Race::send(SendOptions options, std::vector<uint8_t> data) {
    ...
    std::promise<ApiStatus> promise;
    auto future = promise.get_future();
    core->getApiManager().send(options, data,
                               [&promise](ApiStatus status) { promise.set_value(status); });
    return future.get();
}
...
std::pair<ApiStatus, std::vector<uint8_t>> Race::send_receive(SendOptions options,
                                                              std::vector<uint8_t> data) {
    TRACE_METHOD();
    std::promise<std::pair<ApiStatus, std::vector<uint8_t>>> promise;
    auto future = promise.get_future();
    core->getApiManager().sendReceive(options, data,
...
std::pair<ApiStatus, std::string> Race::send_receive_str(SendOptions options, std::string message) {
    TRACE_METHOD();
    std::vector<uint8_t> bytes{message.begin(), message.end()};
...
std::pair<ApiStatus, ConnectionObject> Race::dial(SendOptions options, std::vector<uint8_t> bytes) {
    TRACE_METHOD();
    std::promise<std::tuple<ApiStatus, RaceHandle>> promise;
    auto future = promise.get_future();

    core->getApiManager().dial(options, bytes, [&promise](ApiStatus status, RaceHandle handle) {
...
std::pair<ApiStatus, ConnectionObject> Race::dial_str(SendOptions options, std::string message) {
    TRACE_METHOD();
    std::vector<uint8_t> bytes{message.begin(), message.end()};
