...
Storage::read(const std::filesystem::path &path) const {
  TRACE_METHOD();
  ...
  try {
    ...
    if (file.is_open()) {
      ...
      if (fileSize != fileContents.size()) {
        helper::logError(logPrefix + " read " +
                         std::to_string(fileContents.size()) + " of " +
...
bool Storage::write(const std::filesystem::path &path,
...
bool Storage::append(const std::filesystem::path &path,
                     const std::vector<std::uint8_t> &data) {
  TRACE_METHOD();
  try {
...
std::vector<std::uint8_t>
Storage::decrypt(const std::vector<std::uint8_t> &ciphertext) const {
  return ciphertext;
}

std::vector<std::uint8_t>
Storage::encrypt(const std::vector<std::uint8_t> &plaintext) const {
  return plaintext;
}

bool Storage::appendCiphertext(
    const std::filesystem::path &existingEncryptedFile,
  ...
  try {
    ...
    if (file.is_open()) {
      ...
      combinedPlainText.insert(combinedPlainText.end(), plaintext.begin(),
                               plaintext.end());
