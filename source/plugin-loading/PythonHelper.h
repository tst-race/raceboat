...
/**
 * @brief Initialize the python interpreter. Loads the python interpreter from
 * the location specified by pythonHome. Adds plugin module path and shims path
 * to the python path so plugins and shims can be loaded. No-op if called
 * multiple times.
 *
 ...
 * @return true
 */
bool initPython(std::string pythonHome, std::string pluginModulePath,
                std::string shimsPath);

/**
 * @brief Log a Python error. This function should be passed the resulting
 * `value` from `PyErr_Fetch`.
 *
...
/**
 * @brief Check if a Python error occurred. If it did, throw an exception
