...
public:
     ...
     *
     * @param FileSystem fs used to specify plugins install path and list plugin directories
     * /<plugin id>/ directory and contains manifest.json (eg \p pluginsPath/PluginCommsTwoSixCpp)
     *
    ...
    struct ChannelParameter {
        ...
        ValueType valueType;
        std::string key;
        std::string plugin;        // considered common to all plugins (vs plugin-specific) if empty
