...
CompositeWrapper::CompositeWrapper(
    PluginContainer &container, Core &, Composition composition,
  ...
  auto componentManager = std::make_shared<ComponentManager>(
      *this->getSdk(), composition, transport, usermodel, encodings);

  this->mPlugin = componentManager;
  this->mDescription = description;
}

CompositeWrapper::~CompositeWrapper() { TRACE_METHOD(); }
} // namespace Raceboat
