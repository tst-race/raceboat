...
void DecomposedPluginLoader::loadComponentsForPlugin(
    const PluginDef &pluginToLoad) {
  ...
  if (pluginToLoad.fileType == RaceEnums::PFT_SHARED_LIB) {
    fullPluginPath = fs.makePluginInstallPath(pluginToLoad.sharedLibraryPath,
                                              pluginToLoad.filePath);
    helper::logDebug(logPrefix +
                     "Loading component shared library plugin from " +
                     fullPluginPath);
    plugin = std::make_unique<ComponentPlugin>(
        const_cast<std::string &>(fullPluginPath));
    ...
    plugin = std::make_unique<PythonComponentPlugin>(
        ...
        fs.makePluginInstallBasePath().string(),
        fs.makeShimsPath("python").string());
    helper::logDebug(logPrefix + "Loading component python plugin from " +
                     fullPluginPath);
#else
...
void DecomposedPluginLoader::loadComponents(
    std::vector<PluginDef> componentPlugins) {
  TRACE_METHOD();
...
DecomposedPluginLoader::compose(Composition composition, Core &core) {
  TRACE_METHOD();
  ...
  container->plugin = std::make_unique<CompositeWrapper>(
      *container, core, composition, description, transport, usermodel,
      compositeEncodings);
