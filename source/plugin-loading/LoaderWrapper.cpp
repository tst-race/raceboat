...
LoaderWrapper::LoaderWrapper(PluginContainer &container, Core &,
                             const fs::path &path)
  ...
  debugMessage << logPrefix + "Loading plugin: " << path
               ...
               << ". ID: " << pluginId << ". Description: " << pluginDesc;
  helper::logDebug(debugMessage.str());

  if (version != RACE_VERSION) {
    const std::string errorMessage =
        logPrefix + "Mismatched RACE version number. Expected " +
  ...
  if (pluginId == nullptr || pluginId[0] == '\0') {
    const std::string errorMessage =
        logPrefix + "Invalid plugin ID: null or emptry string.";
    helper::logError(errorMessage);
    throw std::runtime_error(errorMessage);
  }
  ...
  if (pluginId != container.id) {
    const std::string errorMessage =
        logPrefix +
        "Plugin Id does not match expected value. Expected: " + container.id +
        " Got: " + pluginId;
    helper::logError(errorMessage);
    throw std::runtime_error(errorMessage);
  }
