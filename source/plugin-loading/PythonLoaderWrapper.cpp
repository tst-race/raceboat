 ...
 */
static void *createPythonPlugin(IRaceSdkComms *sdk, const std::string &pythonModule,
    ...
    PyTuple_SetItem(new_pArgs, 0, SWIG_NewPointerObj(sdk, SWIG_TypeQuery("IRaceSdkComms*"), 0));
...
std::shared_ptr<IRacePluginComms> createPythonPluginSharedPtr(IRaceSdkComms *sdk,
                                                              const PluginDef &pluginDef) {
    return std::shared_ptr<IRacePluginComms>(
        static_cast<IRacePluginComms *>(
            createPythonPlugin(sdk, pluginDef.pythonModule, pluginDef.pythonClass)),
