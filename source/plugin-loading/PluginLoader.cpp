 ...
 */
static std::unique_ptr<PluginContainer> loadPlugin(PluginDef pluginDef,
                                                   Core &core) {
  TRACE_FUNCTION();
  ...
  case RaceEnums::PluginFileType::PFT_PYTHON: {
    ...
    const fs::path fullPluginPath = fileSystem.makePluginInstallPath(
        pluginDef.sharedLibraryPath, pluginDef.filePath);
    helper::logDebug(logPrefix +
                     "loading Python plugin: " + fullPluginPath.string());
    try {
      container->plugin =
          std::make_unique<PythonLoaderWrapper>(*container, core, pluginDef);
    } catch (const std::exception &e) {
      helper::logError(logPrefix + "Exception loading plugin " +
                       fullPluginPath.string() + ": " + e.what());
  ...
  case RaceEnums::PluginFileType::PFT_SHARED_LIB: {
    const fs::path fullPluginPath = fileSystem.makePluginInstallPath(
        pluginDef.sharedLibraryPath, pluginDef.filePath);
    helper::logDebug(logPrefix + "loading shared library plugin: " +
                     fullPluginPath.string());
    try {
      container->plugin = std::make_unique<LoaderWrapper>(
          *container, core, fullPluginPath.string());
    } catch (const std::exception &e) {
      helper::logError(logPrefix + "Exception loading plugin " +
                       fullPluginPath.string() + ": " + e.what());
      // Don't set container->plugin
...
public:
  PluginLoader(Core &core)
      : core(core),
        decomposedPluginLoader(
            std::make_unique<DecomposedPluginLoader>(core.getFS())) {
    ...
    for (auto &manifest : core.getConfig().manifests) {
      for (auto &composition : manifest.compositions) {
        helper::logDebug("Decomposed channel: " + composition.id +
                         " available");
        auto loader = std::make_shared<LoadableComposition>(
            *decomposedPluginLoader, composition);
  ...
  virtual PluginContainer *getChannel(ChannelId channelId) {
    ...
    if (it != channelMap.end()) {
      ...
      // pluginPtr is guaranteed to remain valid. We only needed the lock for
      // accessing the channel map, and we don't want to hold it while calling
      // into the plugin. Init is synchronous and blocks until the plugin
      // returns.
      lock.unlock();
...
protected:
  PluginConfig getPluginConfig(PluginContainer &pluginContainer) {
    ...
    pluginConfig.etcDirectory =
        core.getFS().makeRaceDir("etc", pluginContainer.id);
    pluginConfig.loggingDirectory =
        core.getFS().makeRaceDir("logging", pluginContainer.id);
    pluginConfig.auxDataDirectory =
        core.getFS().makeRaceDir("aux", pluginContainer.id);
    pluginConfig.pluginDirectory =
        core.getFS().makePluginInstallPath("", pluginContainer.id);

    // TODO: use system tmp directory?
    pluginConfig.tmpDirectory =
        core.getFS().makeRaceDir("tmp", pluginContainer.id);
    return pluginConfig;
  }
...
protected:
  ...
  // All access to either the plugin vector or the channel map after
  // initialization should be protected by this mutex
  ...
  std::unordered_map<ChannelId, std::shared_ptr<LoadablePlugin>>
      channelPluginLoaderMap;
};
