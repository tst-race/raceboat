...
public:
  ...
  void close() noexcept;
  bool is_loaded() const noexcept { return lib != nullptr; }
  ...
  template <typename T> T &get(T *&result, const char *name) const {
    result = &get<T>(name);
    return *result;
  }
  template <typename T> T &get(T *&result, const std::string &name) const {
  ...
  explicit DynamicLibrary(const path_string &path, bool global = false)
      : DynamicLibrary(path.c_str(), global) {}
  ...
  DynamicLibrary(DynamicLibrary &&o) noexcept { std::swap(lib, o.lib); }
...
private:
  ...
  // store library name for logging if somthing goes wrong
  std::string lib_path;
...
inline void DynamicLibrary::open(const DynamicLibrary::path_char *path,
                                 bool global) {
  lib_path = path;
  helper::logDebug(std::string("opening library: ") + lib_path);
  ...
  if (lib == nullptr) {
    ...
    const std::string errorMessage =
        ...
        std::string(dlerrorMessage != nullptr ? dlerrorMessage
                                              : "unknown error");
...
template <typename T> inline T &DynamicLibrary::get(const char *name) const {
  ...
  if (ptr == nullptr) {
    const std::string errorMessage =
        "DynamicLibrary::get: Symbol not found: '"s + name + "' in library: '" +
        lib_path + "'";
    helper::logError(errorMessage);
    throw std::runtime_error(errorMessage);
  }
