...
#else
  return ::dlopen(
      path, RTLD_LAZY | (global ? RTLD_GLOBAL : RTLD_LOCAL | RTLD_DEEPBIND));
#endif
}

void DynamicLibrary::dlclose(void *lib) noexcept { ::dlclose(lib); }

void *DynamicLibrary::dlsym(void *lib, const char *name) noexcept {
  return ::dlsym(lib, name);
}

char *DynamicLibrary::dlerror() noexcept { return ::dlerror(); }
...
void DynamicLibrary::dlclose(void *lib) noexcept { FreeLibrary((HMODULE)lib); }

void *DynamicLibrary::dlsym(void *lib, const char *name) noexcept {
  return (void *)GetProcAddress((HMODULE)lib, (LPCSTR)name);
}

char *DynamicLibrary::dlerror() noexcept { return GetLastError(); }
