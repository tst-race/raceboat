...
bool PluginDef::is_decomposed_plugin() const { return !is_unified_plugin(); }

bool PluginDef::is_unified_plugin() const { return !channels.empty(); }

void from_json(const nlohmann::json &pluginJson, PluginDef &pluginDef) {
  try {
    pluginDef.fileType =
        RaceEnums::stringToPluginFileType(pluginJson.at("file_type"));
    ...
    pluginDef.channels =
        pluginJson.value("channels", std::vector<std::string>{});
    pluginDef.transports =
        pluginJson.value("transports", std::vector<std::string>{});
    pluginDef.usermodels =
        pluginJson.value("usermodels", std::vector<std::string>{});
    pluginDef.encodings =
        pluginJson.value("encodings", std::vector<std::string>{});
