...
// This implementation provides the basis to traverse states upon internal and
// external events Priorities (descending): readability, ease of use,
// flexibility/expressiveness Given a State and a Context: events are the
// stimulus to transition to another State Users declare States and transitions
// so the intention can be easily inferred
...
// - All States should be able to transition to another State unless
// State::finalState()
// - StateEngine::validateStateMachine() identifies problems with state
// transition declarations
//
// - External events are handled via StateEngine::handleEvent()
// - Internal events can be added to Context::pendingEvents by State virtual
// methods called in handleEvent()
//
// - All stateful info is in Context, no stateful info is in State or
// StateEngine.
//   This allows use of just one State instance per state throughout the
//   lifecycle of the StateEngine as opposed to tasking each State
//   implementation with creating instances of states resulting from events
...
struct Context;
template <class T> T *getDerivedContext(Context *context) {
  T *derived = nullptr;
  if (context) {
    try {
      derived = dynamic_cast<T *>(context);
    } catch (const std::exception &ex) {
      helper::logError("could not derive " + std::string(typeid(T).name()) +
                       " from Context");
    }
  } else {
...
struct State {
  ...
  virtual EventResult enter(Context &) { return EventResult::SUCCESS; }
  virtual EventResult exit(Context &) { return EventResult::SUCCESS; }
    ...
    try {
      ...
      if (result != EventResult::SUCCESS) {
        helper::logError(logPrefix +
                         " failed with return : " + EventResultString(result));
      }
      return result;
  ...
  virtual bool prerequisitesSatisfied(Context &) { return true; }

  virtual StateType
  nextStateId(Context &context, EventType eventId,
              const std::unordered_set<StateType> &allToStates);

  // return true for no transitions
  virtual bool finalState() { return false; }
...
public:
  ...
  template <class ClassT> void addState(StateType stateId) {
    addState(stateId, std::make_shared<ClassT>(stateId));
  }
  template <class ClassT> void addInitialState(StateType stateId) {
    std::shared_ptr<ClassT> state = std::make_shared<ClassT>(stateId);
    initStateId = stateId;
    addState(stateId, state);
  }
  template <class ClassT> void addFailedState(StateType stateId) {
    std::shared_ptr<ClassT> state = std::make_shared<ClassT>(stateId);
  ...
  void declareStateTransition(StateType fromStateId, EventType eventId,
                              StateType toStateId);
...
protected:
  ...
  EventResult transitionToState(Context &context, State &currState,
                                State &nextState);
