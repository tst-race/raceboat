...
void ApiConnContext::updateConnStateMachineStart(RaceHandle _contextHandle, ChannelId _channelId,
                                                 std::string _role, std::string _linkAddress,
                                                 bool _sending) {
...
void ApiConnContext::updateChannelStatusChanged(RaceHandle /* _chanHandle */,
                                                const ChannelId & /* _channelGid */,
                                                ChannelStatus /* _status */,
                                                const ChannelProperties & /* _properties */) {}
void ApiConnContext::updateLinkStatusChanged(RaceHandle /* _linkHandle */, const LinkID &_linkId,
                                             LinkStatus /* _status */,
                                             const LinkProperties &_properties) {
    this->linkId = _linkId;
    this->updatedLinkAddress = _properties.linkAddress;
}
void ApiConnContext::updateConnectionStatusChanged(RaceHandle /* _connHandle */,
                                                   const ConnectionID &_connId,
...
struct StateConnInitial : public ConnState {
    ...
    virtual EventResult enter(Context &context) {
        ...
        // listen to events from the parent (a recv/send state machine). This will cause
        // EVENT_CONN_CLOSE, EVENT_STATE_MACHINE_FINISHED, EVENT_STATE_MACHINE_FAILED to be
        // forwarded to us if triggered by the parent
        ctx.manager.registerHandle(ctx, ctx.newestDependent);

        RaceHandle chanHandle = ctx.manager.getCore().generateHandle();
        auto response =
            ctx.manager.activateChannel(ctx, chanHandle, ctx.channelId, ctx.channelRole);
...
struct StateConnActivated : public ConnState {
    ...
    virtual EventResult enter(Context &context) {
        ...
        if (ctx.send && ctx.linkAddress.empty()) {
            ...
            response = plugin.loadLinkAddress(linkHandle, ctx.channelId, ctx.linkAddress, 0);
        } else if (ctx.linkAddress.empty()) {
            response = plugin.createLink(linkHandle, ctx.channelId, 0);
        } else {
            response = plugin.createLinkFromAddress(linkHandle, ctx.channelId, ctx.linkAddress, 0);
        }
...
struct StateConnLinkEstablished : public ConnState {
    ...
    virtual EventResult enter(Context &context) {
        ...
        if (!ctx.send && !ctx.linkAddress.empty() && ctx.updatedLinkAddress != ctx.linkAddress) {
            ...
            if (updatedJson != originalJson) {
                helper::logError(logPrefix +
                                 "received link address does not match requested link address supplied "
                                 "by user. Requested: " +
        ...
        SdkResponse response =
            plugin.openConnection(openConnHandle, linkType, ctx.linkId, "{}", 0, 0, 0);
...
struct StateConnConnectionOpen : public ConnState {
    ...
    virtual EventResult enter(Context &context) {
        ...
        ctx.manager.connStateMachineConnected(ctx.handle, ctx.connId, ctx.updatedLinkAddress);
...
struct StateConnClosing : public ConnState {
    ...
    virtual EventResult enter(Context &context) {
        ...
        if (context.currentStateId == stateId) {
            // This can loop back to the same state if a EVENT_PACKAGE_RECEIVED happens
        ...
        SdkResponse response = plugin.closeConnection(closeConnHandle, ctx.connId, 0);
...
struct StateConnConnectionClosed : public ConnState {
    explicit StateConnConnectionClosed(StateType id = STATE_CONN_CONNECTION_CLOSED) :
...
ConnStateEngine::ConnStateEngine() {
    // calls activateChannel on plugin, waits for onChannelStatusChanged(status=CHANEL_ACTIVATED)
    addInitialState<StateConnInitial>(STATE_CONN_INITIAL);

    // calls load/create link on plugin, waits for onLinkStatusChanged(status=LINK_CREATED/LOADED)
    ...
    // Tell the parent (e.g. a send or a recv) state machine that we're connected and always
    ...
    // calls destroyLink on the plugin, wait for onLinkStatusChanged(status=LINK_DESTROYED)
