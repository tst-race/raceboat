...
void ApiSendReceiveContext::updateConnStateMachineConnected(RaceHandle contextHandle,
                                                            ConnectionID connId,
...
void ApiSendReceiveContext::updateReceiveEncPkg(ConnectionID /* _connId */,
...
struct StateSendReceiveWaitingForSecondConnection : public SendReceiveState {
    explicit StateSendReceiveWaitingForSecondConnection(
        ...
        SendReceiveState(id, "STATE_SEND_RECEIVE_WAITING_FOR_SECOND_CONNECTION") {}
};

struct StateSendReceiveConnectionsOpen : public SendReceiveState {
    explicit StateSendReceiveConnectionsOpen(StateType id = STATE_SEND_RECEIVE_CONNECTIONS_OPEN) :
...
struct StateSendReceivePackageSent : public SendReceiveState {
    explicit StateSendReceivePackageSent(StateType id = STATE_SEND_RECEIVE_PACKAGE_SENT) :
...
SendReceiveStateEngine::SendReceiveStateEngine() {
    ...
    // calls sendPackage on plugin, waits for onPackageStatusChanged(status=PACKAGE_SENT)
    addState<StateSendReceiveConnectionsOpen>(STATE_SEND_RECEIVE_CONNECTIONS_OPEN);
    // does nothing, waits for a receiveEncPkg call
    addState<StateSendReceivePackageSent>(STATE_SEND_RECEIVE_PACKAGE_SENT);
    // calls callback with received message, calls state machine finished on manager, final state
