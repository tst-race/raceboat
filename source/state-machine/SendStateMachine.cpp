...
void ApiSendContext::updateSend(const SendOptions &_sendOptions, std::vector<uint8_t> &&_data,
                                std::function<void(ApiStatus)> _cb) {
    this->opts = _sendOptions;
    this->data = _data;
    this->callback = _cb;
}
void ApiSendContext::updateConnStateMachineConnected(RaceHandle /* _contextHandle */,
...
struct StateSendInitial : public SendState {
    ...
    virtual EventResult enter(Context &context) {
        ...
        RaceHandle connStateMachineHandle =
            ctx.manager.startConnStateMachine(ctx.handle, channelId, role, linkAddress, true);
