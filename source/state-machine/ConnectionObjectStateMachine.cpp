...
void ConnectionObjectContext::updateConnObjectStateMachineStart(
    ...
    RaceHandle sendHandle, const ConnectionID &_sendConnId, const ChannelId &_sendChannel,
...
void ConnectionObjectContext::updateReceiveEncPkg(ConnectionID /* connId */,
...
void ConnectionObjectContext::updateRead(RaceHandle /* handle */,
...
void ConnectionObjectContext::updateWrite(RaceHandle /* handle */, std::vector<uint8_t> bytes,
...
struct StateConnectionObjectInitial : public ConnectionObjectState {
    explicit StateConnectionObjectInitial(StateType id = STATE_CONNECTION_OBJECT_INITIAL) :
    ...
    virtual EventResult enter(Context &context) {
        ...
        std::vector<uint8_t> packageIdBytes{ctx.packageId.begin(), ctx.packageId.end()};
...
struct StateConnectionObjectConnected : public ConnectionObjectState {
    explicit StateConnectionObjectConnected(StateType id = STATE_CONNECTION_OBJECT_CONNECTED) :
    ...
    virtual EventResult enter(Context &context) {
        ...
        for (auto &[cb, bytes] : ctx.sendQueue) {
            ...
            prefixedBytes.insert(prefixedBytes.end(), ctx.packageId.begin(), ctx.packageId.end());
...
struct StateConnectionObjectFinished : public ConnectionObjectState {
    explicit StateConnectionObjectFinished(StateType id = STATE_CONNECTION_OBJECT_FINISHED) :
...
struct StateConnectionObjectFailed : public ConnectionObjectState {
    explicit StateConnectionObjectFailed(StateType id = STATE_CONNECTION_OBJECT_FAILED) :
...
ConnectionObjectStateEngine::ConnectionObjectStateEngine() {
    // calls sendPackage on plugin with any package in queue, re-enters state if read, write, or
    // receiveEncPkg is called, transitions on close call
    addInitialState<StateConnectionObjectInitial>(STATE_CONNECTION_OBJECT_INITIAL);
