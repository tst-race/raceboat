...
void ApiRecvContext::updateReceive(RaceHandle /* _handle */,
                                   std::function<void(ApiStatus, std::vector<uint8_t>)> _cb) {
    this->receiveCb = _cb;
};
void ApiRecvContext::updateClose(RaceHandle /* handle */, std::function<void(ApiStatus)> _cb) {
    this->closeCb = _cb;
}

void ApiRecvContext::updateReceiveEncPkg(ConnectionID /* _connId */,
                                         std::shared_ptr<std::vector<uint8_t>> _data) {
    this->data.push(std::move(_data));
};
void ApiRecvContext::updateConnStateMachineConnected(RaceHandle /* _contextHandle */,
...
struct StateRecvInitial : public RecvState {
    ...
    virtual EventResult enter(Context &context) {
        ...
        RaceHandle connStateMachineHandle =
...
struct StateRecvWaitingForAppAndPlugin : public RecvState {
    explicit StateRecvWaitingForAppAndPlugin(StateType id = STATE_RECV_WAITING_FOR_APP_AND_PLUGIN) :
...
struct StateRecvWaitingForPlugin : public RecvState {
    explicit StateRecvWaitingForPlugin(StateType id = STATE_RECV_WAITING_FOR_PLUGIN) :
...
RecvStateEngine::RecvStateEngine() {
    ...
    // waits for a receive() call on the receiver, a recvEncPkg call from plugin, or a close call
    // from the app
    addState<StateRecvWaitingForAppAndPlugin>(STATE_RECV_WAITING_FOR_APP_AND_PLUGIN);
    // wait for a receive() call or a close() call from the app
    // If another packages is received here, it's added to a queue, but no transition is made
    ...
    // call user callback with received package, transitions to WAITING_FOR_APP_AND_PLUGIN or
    // WAITING_FOR_APP based on if there's still packages in the queue
