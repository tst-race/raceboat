...
StateType State::nextStateId(Context &, EventType eventId,
  ...
  if (allToStates.size() == 1) {
    ...
    helper::logError(
        logPrefix + "event " + std::to_string(eventId) + " can transition to " +
...
StateEngine::StateEngine()
    : initStateId(STATE_INVALID), failedStateId(STATE_INVALID) {}

void StateEngine::declareInitialTransition(EventType eventId,
                                           StateType toStateId) {
  declareStateTransition(initStateId, eventId, toStateId);
}

void StateEngine::declareStateTransition(StateType fromStateId,
                                         EventType eventId,
                                         StateType toStateId) {
...
void StateEngine::addState(StateType stateId, std::shared_ptr<State> state) {
  if (idToInstanceMap.find(stateId) != idToInstanceMap.end()) {
    MAKE_LOG_PREFIX();
    helper::logError(logPrefix + "over-writing pre-existing state " +
...
bool StateEngine::contextValid(Context &context, const std::string &logPrefix) {
  if (idToInstanceMap.find(context.currentStateId) == idToInstanceMap.end()) {
    helper::logError(logPrefix + " invalid state " +
                     stateToString(context.currentStateId));
    return false;
  }
  return true;
}
...
bool StateEngine::stateExists(StateType stateId) {
  ...
  if (stateIdsIt == idToInstanceMap.end()) {
    helper::logDebug(logPrefix + " no state for state " +
                     std::to_string(stateId));
    return false;
  } else if (stateIdsIt->second.get() == nullptr) {
    helper::logDebug(logPrefix + " null state for state " +
                     std::to_string(stateId));
    return false;
  }
  return true;
...
EventResult StateEngine::handleEvents(Context &context) {
  ...
  while (!context.pendingEvents.empty()) {
    ...
    if (stateHandlesEvent(context.currentStateId, eventId)) {
      ...
      result =
          EventResult::NOT_SUPPORTED; // not supported if not in transition map
      helper::logError(logPrefix + " state " +
                       stateToString(context.currentStateId) +
...
bool StateEngine::stateHandlesEvent(StateType stateId, EventType eventId) {
  ...
  if (stateExists(stateId)) {
    if (validTransitions[stateId].find(eventId) !=
        validTransitions[stateId].end()) {
      success = true;
    } else {
...
EventResult StateEngine::transitionToState(Context &context, State &curr,
                                           State &next) {
  ...
  if (not next.prerequisitesSatisfied(context)) {
    helper::logError(logPrefix + " next state " + stateToString(next.stateId) +
                     " not ready");
    return EventResult::NOT_SUPPORTED;
  }
  ...
  if (result == EventResult::NOT_SUPPORTED) {
    helper::logError(logPrefix + " failed to enter next state " +
                     stateToString(next.stateId));
  } else {
...
StateType StateEngine::getNextStateId(Context &context, State &currentState,
                                      EventType eventId) {
  ...
  if (stateEventsPair != validTransitions.end()) {
    ...
    if (eventToStatesPair != stateEventsPair->second.end()) {
      nextStateId =
          currentState.nextStateId(context, eventId, eventToStatesPair->second);
    }
  }
  return nextStateId;
}
...
bool StateEngine::validateStateMachine() {
  ...
  for (auto idInstPair : idToInstanceMap) {
    // every from-state must be registered in validTransitions except FAILED and
    // final states
    if (idInstPair.first != failedStateId &&
        not idInstPair.second->finalState() &&
      ...
      for (auto eventToStateSetPair : stateEventsMapPair.second) {
        if (eventToStateSetPair.second.empty()) {
          ...
          helper::logError(logPrefix + "event Id " +
                           eventToString(eventToStateSetPair.first) +
                           " for state " +
                           stateToString(stateEventsMapPair.first) +
        ...
        for (auto toState : eventToStateSetPair.second) {
          ...
          if (toState == STATE_INVALID) {
            success = false;
            helper::logError(logPrefix + "invalid to-state " +
  ...
  for (auto idEnteredpair : toStateTransitions) {
    if (idEnteredpair.second == false && idEnteredpair.first != initStateId &&
      ...
      helper::logError(logPrefix + " Non-init state (" +
                       stateToString(idEnteredpair.first) +
