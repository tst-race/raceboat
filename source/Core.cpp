...
Core::Core()
    : fs(""), userInput({}), apiManager(std::make_unique<ApiManager>(*this)) {}
Core::Core(std::string pluginPath, ChannelParamStore params)
    : fs(pluginPath), userInput(params),
      apiManager(std::make_unique<ApiManager>(*this)) {
  init();
}
Core::~Core() {
  shuttingDown = true;
...
void Core::init() {
  if (!config.parsePluginManifests(getFS())) {
    std::string message = "Unable to parse any plugin manifests in path: " +
                          getFS().pluginsInstallPath.string();
    helper::logError(message);
  }

  // plugin loader and channelManager require config to be populated before
  // initializing
...
const UserInput &Core::getUserInput() { return userInput; }

const Config &Core::getConfig() { return config; }

ChannelManager &Core::getChannelManager() { return *channelManager; }

ApiManager &Core::getApiManager() { return *apiManager; }
...
SdkResponse Core::asyncError(PluginContainer & /* plugin */,
                             RaceHandle /* handle */,
...
SdkResponse Core::onPackageStatusChanged(PluginContainer &plugin,
                                         RaceHandle handle,
...
SdkResponse Core::onConnectionStatusChanged(PluginContainer &plugin,
                                            RaceHandle handle,
  ...
  return getApiManager().onConnectionStatusChanged(plugin, handle, connId,
                                                   status, properties);
}

SdkResponse Core::onLinkStatusChanged(PluginContainer &plugin,
  ...
  return getApiManager().onLinkStatusChanged(plugin, handle, linkId, status,
                                             properties);
}

SdkResponse Core::onChannelStatusChanged(PluginContainer &plugin,
                                         RaceHandle handle,
  ...
  getChannelManager().onChannelStatusChanged(handle, channelGid, status,
                                             properties);
  return getApiManager().onChannelStatusChanged(plugin, handle, channelGid,
                                                status, properties);
}

SdkResponse Core::updateLinkProperties(PluginContainer &, const LinkID &,
...
ConnectionID Core::generateConnectionId(PluginContainer & /* plugin */,
                                        const LinkID &linkId) {
  static std::atomic<int> connectionCount = 0;
  return linkId + "/Connection_" + std::to_string(connectionCount++);
}

LinkID Core::generateLinkId(PluginContainer &plugin,
                            const ChannelId &channelGid) {
  static std::atomic<int> linkCount = 0;
  return plugin.id + "/" + channelGid + "/LinkID_" +
