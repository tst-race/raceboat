...
struct FileSystem {
     ...
     *
     * @param pluginsInstallPath absolute plugins install directory (eg /opt/race/)
     * @param storage (or derived class) used to perform file IO
     **/
    explicit FileSystem(const fs::path &pluginsInstallPath,
     ...
     */
    virtual bool makeDir(const fs::path &directoryPath, const std::string &pluginId);
     ...
     */
    virtual bool removeDir(const fs::path &directoryPath, const std::string &pluginId);
    ...
    /**
     * @brief Construct a filepath based on a filepath, pluginId, and data directory path for plugin
     * sandbox directory (eg < \p pluginsInstallPath>/usr/< \p pluginId>/< \p filePath>)
     *
     ...
     */
    virtual fs::path makePluginFilePath(const fs::path &filePath, const std::string &pluginId);

    /**
     * @brief Construct the anticipated plugin install filepath based on a filepath, pluginId,
     * and data directory path for plugin binary directory
     * (eg < \p pluginsInstallPath>/plugins/<hostOS>/<hostArch>/< \p pluginId>/< \p filePath>)
     *
     * @param filePath The string name of the file
     * @param pluginId The string pluginId the file is associated with
     * @return fs::path Full path to the filepath
     */
    virtual fs::path makePluginInstallPath(const fs::path &filePath, const std::string &pluginId);
     ...
     * @return fs::path Full path to the filepath
     */
    virtual fs::path makeRaceDir(const fs::path &prefix, const std::string &pluginId);
     ...
     * @return std::vector<fs::path> all directories in user-specified plugin install path
     */
