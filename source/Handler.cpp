...
// increment the iterator to the next value. If it has reached the end of the
// container, reset it to the beginning
template <typename I, typename C> I next_cycle(I iter, C &container) {
  iter++;
...
Handler::Handler(const std::string &name, size_t _max_queue_size,
                 size_t _max_total_size)
    : name(name), max_queue_size(_max_queue_size),
      max_total_size(_max_total_size), state(State::PRESTART),
      timeoutQueue(
          [](const std::shared_ptr<Work> &w1, const std::shared_ptr<Work> &w2) {
            return w1->timeoutTimestamp < w2->timeoutTimestamp;
          }),
      total_work(0), total_marked(0), total_size(0), unblocked_work(0) {
  create_queue("", 0);
...
void Handler::start() {
  ...
  if (state.load() != State::PRESTART) {
    throw std::logic_error("Failed to start handler thread. Handler was not in "
                           "a valid state for start.");
  }
...
void Handler::runTimeoutThread() {
  ...
  while (true) {
    ...
    if (timeoutTimestamp < std::numeric_limits<double>::infinity()) {
      ...
      // special case infinity. Waiting until infinity returns immediately, when
      // we want it to wait until it gets a signal
    ...
    while (timeoutQueue.size() > 0) {
      ...
      helper::logDebug(
          "Handler::runTimeoutThread: Moving work to timed out queue");
    }
...
void Handler::runWorkThread() {
  ...
  while (true) {
    ...
    work_thread_signaler.wait(lock, [this] {
      return unblock_list.size() > 0 || state.load() != State::STARTED ||
             total_marked > 0 || unblocked_work > 0 || timedOutQueue.size() > 0;
    });

    // unblock any queues. This must happen before the check to stop, since this
    // may cause new work to become available.
    ...
    // got the queue to update, either get work from it, or remove it as
    // necessary
    if (!queue->queue.empty()) {
      ...
      if (success) {
        ...
        // item is blocked. Don't pop, since we want it to be there when we're
        // unblocked
...
void Handler::stop() {
  ...
  if (prev_state == State::STARTED) {
    ...
    if (unblocked_work != 0) {
      helper::logError(
          "Handler::stop: handler has work remaining after stop()");
    }
  }
...
void Handler::create_queue(const std::string &queue_name, int priority) {
  ...
  PriorityIter priorityIter =
      priority_levels.emplace(priority, PriorityLevel(priority)).first;
...
void Handler::remove_queue(const std::string &queue_name) {
  ...
  // sanity check. Run assumes there is at least on queue in existence. Keep
  // this queue around to ensure that.
  if (queue_name == "") {
  ...
  if (queue->blocked == false) {
    ...
    // There's a new marked queue, so tell the work thread to wake if if it's
    // not awake already
    work_thread_signaler.notify_one();
  }
}
...
void Handler::unblock_queue(const std::string &queue_name) {
  ...
  // push on to list to be run by work thread. We don't want to unblock stuff
  // while a callback is being run. There's a potential race condition where
  // callback determines it is blocked, something signals the unblock, we get
  // the unblock and block after the callback returned in that order and end up
  // blocked when we shouldn't be if the callback and unblocking isn't
  // synchronized.
  unblock_list.push_back(queue_name);

  // There's a new queue to unblock, so tell the work thread to wake if if it's
  // not awake already
  work_thread_signaler.notify_one();
}

Handler::State Handler::get_state() { return state.load(); }
...
void Handler::join_work_thread() {
  ...
  // Wait for the threads to join. If they don't join within the timeout then
  // assume they are hanging, log an error, and bail.
  ...
  if (status != std::future_status::ready) {
    helper::logError("FATAL: Handler::join_work_thread: timed out waiting for "
...
void Handler::join_timeout_thread() {
  ...
  auto status = timeoutPromise.get_future().wait_for(
      std::chrono::seconds(secondsToWaitForThreadsToJoin));
  if (status != std::future_status::ready) {
    helper::logError("FATAL: Handler::join_timeout_thread: timed out waiting "
                     "for timeout thread to join. "
                     "Terminating.");
    std::terminate();
  }
}
...
void Handler::unblock_queue_internal(const std::string &queue_name) {
  ...
  if (queue->blocked == true) {
    ...
    // If this was marked for removal, add it to the marked counts. Queues that
    // are blocked don't count towards that. Now that it's unblocked, it should
    // count towards the number of marked queues
...
Handler::QueueIter &Handler::get_next_queue_with_work() {
  ...
  QueueIter &queue = queueIters
                         .emplace(current_priority->first,
                                  current_priority->second.work_queues.begin())
                         .first->second;

  // find next queue that is not blocked and has work or is marked and deal with
  // it.
...
void Handler::remove_queue_internal(Handler::QueueIter &queue) {
  ...
  // remove this queue from the list of queues and advance the iterator to the
  // next queue
  QueueIter next_queue =
      next_cycle(queue, current_priority->second.work_queues);
  ...
  if (current_priority->second.work_queues.empty()) {
    ...
    if (priority_levels.empty()) {
      // The logic for finding work assumes the current priority is valid. If
      // this is reached, there are no valid priorities left. In remove queue,
      // we prevent removing the default queue to prevent this from happening.
      helper::logError(
          "Handler::run No queues left. This is an invalid state.");
    }
  }
}
