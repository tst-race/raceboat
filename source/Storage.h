...
public:
   ...
   * @return true if successful, otherwise false
   */
  bool append(const std::filesystem::path &path,
              const std::vector<std::uint8_t> &data);

protected:
  virtual bool isFileEncryptable(const std::filesystem::path &path) const;
  virtual std::vector<std::uint8_t>
  decrypt(const std::vector<std::uint8_t> &ciphertext) const;
  virtual std::vector<std::uint8_t>
  encrypt(const std::vector<std::uint8_t> &plaintext) const;

  // allow user to optimize appending to an encrypted file (implementation
  // specific optimization) base implemenation is rather brute force (read
  // entire file, decrypt, append new plaintext, write)
  virtual bool
  appendCiphertext(const std::filesystem::path &existingEncryptedFile,
