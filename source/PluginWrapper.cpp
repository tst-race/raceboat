...
PluginWrapper::PluginWrapper(PluginContainer &container,
    ...
    : mPlugin(std::move(plugin)), mDescription(std::move(description)),
      container(container), mThreadHandler(container.id + "-thread", 0, 0) {
  mThreadHandler.create_queue("lifecycle", std::numeric_limits<int>::max());
  mThreadHandler.create_queue("wait queue", std::numeric_limits<int>::min());
  mThreadHandler.start();
...
SdkResponse PluginWrapper::post(const std::string &logPrefix,
                                const std::string &queue, T &&function,
                                Args &&... args) {
  ...
  try {
    auto workFunc =
        [logPrefix, postId, function, supportTempError,
      ...
      try {
        // We can't move the arguments that are used for this call because the
        // lambda may be called more than once.
      ...
      helper::logDebug(logPrefix + "Returned " +
                       pluginResponseToString(status) + ", postId: " + postId);
    ...
    auto [success, queueSize, future] = mThreadHandler.post(
        queue, 0, -1,
        std::bind(std::move(workFunc), std::forward<Args>(args)...));
    ...
    return makeResponse(logPrefix, success == Handler::PostStatus::OK,
                        queueSize, 0);
...
template <typename T, typename... Args>
bool PluginWrapper::post_lifecycle(const std::string &logPrefix,
                                   int32_t timeoutInSeconds, T &&function,
                                   Args &&... args) {
  auto postHandle = nextPostId++;
  std::string postId = std::to_string(postHandle);
  helper::logDebug(logPrefix + "Posting postId: " + postId);
  ...
  try {
    auto workFunc = [logPrefix, postId, function,
    ...
    if (timeoutInSeconds == WAIT_FOREVER) {
      ...
      std::future_status status =
          future.wait_for(std::chrono::seconds(timeoutInSeconds));
...
bool PluginWrapper::init(const PluginConfig &pluginConfig) {
  TRACE_METHOD(getId());
  return post_lifecycle(logPrefix, WAIT_FOREVER, &IRacePluginComms::init,
                        pluginConfig);
}

bool PluginWrapper::shutdown() {
  // By default, wait with a 30 second timeout for shutdown to complete. Based
  // on internal discussions this _should_ be ample time for the plugins to
  // shutdown. However, this value may be adjusted after testing with actual
  // plugins. Note that if the work queue grows large when this is called (e.g.
  // shutdown gets called in the middle of a large scale stress test) the
  // timeout may get hit before shutdown even gets called. Leaving this as-is
  // for now as this seems to be a corner case, and should be easy to diagnose
  // from the logs.
...
bool PluginWrapper::shutdown(std::int32_t timeoutInSeconds) {
  ...
  return post_lifecycle(logPrefix, timeoutInSeconds,
                        &IRacePluginComms::shutdown);
}

SdkResponse PluginWrapper::sendPackage(RaceHandle handle,
                                       const ConnectionID &connectionId,
                                       const EncPkg &pkg, int32_t postTimeout,
                                       uint64_t batchId) {
  ...
  return post(logPrefix, queue, &IRacePluginComms::sendPackage, handle,
              connectionId, pkg, std::numeric_limits<double>::infinity(),
              batchId);
}
...
SdkResponse PluginWrapper::openConnection(RaceHandle handle, LinkType linkType,
                                          ...
                                          int32_t) {
  TRACE_METHOD(getId(), handle, linkType, linkId, linkHints, priority,
               sendTimeout);

  if (handle == NULL_RACE_HANDLE) {
    helper::logError(logPrefix + "received invalid handle");
    return SDK_INVALID_ARGUMENT;
  } else if (linkType != LT_BIDI && linkType != LT_SEND &&
             linkType != LT_RECV) {
  ...
  return post(logPrefix, "", &IRacePluginComms::openConnection, handle,
              linkType, linkId, linkHints, sendTimeout);
}

SdkResponse PluginWrapper::closeConnection(RaceHandle handle,
                                           const ConnectionID &connectionId,
                                           int32_t) {
  ...
  return post(logPrefix, "", &IRacePluginComms::closeConnection, handle,
              connectionId);
}

SdkResponse PluginWrapper::deactivateChannel(RaceHandle handle,
  ...
  if (handle == NULL_RACE_HANDLE) {
    ...
    // Ideally, this should be more strict and require the channel to actually
    // be available on this plugin, but there's no easy way to do that
    // currently, so just checking like this.
    helper::logError(logPrefix + "received empty channel id");
    return SDK_INVALID_ARGUMENT;
  }

  return post(logPrefix, "", &IRacePluginComms::deactivateChannel, handle,
              channelGid);
}

SdkResponse PluginWrapper::activateChannel(RaceHandle handle,
                                           const std::string &channelGid,
  ...
  return post(logPrefix, "", &IRacePluginComms::activateChannel, handle,
              channelGid, roleName);
}

SdkResponse PluginWrapper::destroyLink(RaceHandle handle, const LinkID &linkId,
                                       std::int32_t) {
...
SdkResponse PluginWrapper::createLink(RaceHandle handle,
                                      const std::string &channelGid,
                                      std::int32_t) {
  TRACE_METHOD(getId(), handle, channelGid);
...
SdkResponse PluginWrapper::createBootstrapLink(RaceHandle handle,
                                               const std::string &channelGid,
  ...
  return post(logPrefix, "", &IRacePluginComms::createBootstrapLink, handle,
              channelGid, passphrase);
}

SdkResponse PluginWrapper::loadLinkAddress(RaceHandle handle,
                                           const std::string &channelGid,
  ...
  return post(logPrefix, "", &IRacePluginComms::loadLinkAddress, handle,
              channelGid, linkAddress);
}
SdkResponse PluginWrapper::loadLinkAddresses(
  ...
  return post(logPrefix, "", &IRacePluginComms::loadLinkAddresses, handle,
              channelGid, linkAddresses);
}

SdkResponse PluginWrapper::createLinkFromAddress(RaceHandle handle,
                                                 const std::string &channelGid,
                                                 const std::string &linkAddress,
                                                 std::int32_t) {
  TRACE_METHOD(getId(), handle, channelGid, linkAddress);
  ...
  return post(logPrefix, "", &IRacePluginComms::createLinkFromAddress, handle,
              channelGid, linkAddress);
}

SdkResponse PluginWrapper::serveFiles(LinkID linkId, std::string path,
                                      int32_t) {
...
SdkResponse PluginWrapper::flushChannel(RaceHandle handle,
                                        std::string channelGid,
                                        uint64_t batchId, int32_t) {
  ...
  return post(logPrefix, "", &IRacePluginComms::flushChannel, handle,
              channelGid, batchId);
}
...
SdkResponse PluginWrapper::onUserInputReceived(RaceHandle handle, bool answered,
  ...
  return post(logPrefix, "", &IRacePluginComms::onUserInputReceived, handle,
              answered, userResponse);
}

SdkResponse PluginWrapper::onUserAcknowledgementReceived(RaceHandle handle,
                                                         int32_t) {
  ...
  return post(logPrefix, "", &IRacePluginComms::onUserAcknowledgementReceived,
              handle);
}

void PluginWrapper::onConnectionStatusChanged(const ConnectionID &connId,
...
SdkResponse PluginWrapper::makeResponse(const std::string &functionName,
                                        bool success, size_t queueSize,
                                        RaceHandle handle) {
  double queueUtilization =
      (static_cast<double>(queueSize) / mThreadHandler.max_queue_size);
