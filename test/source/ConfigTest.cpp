...
TEST(Config, manifest_parsing) {
    ...
    Config config;
    fs::path pluginsDir = fs::path(INPUT_FILE_DIR) / "plugins" / "myOS" / "myArch";
    std::vector<fs::path> pluginPaths = {pluginsDir / "examplePlugin1",
                                         pluginsDir / "examplePlugin2"};

    printf("plugin path: %s\n", pluginPaths[0].string().c_str());

    EXPECT_CALL(fs, listInstalledPluginDirs()).WillOnce(::testing::Return(pluginPaths));
    ...
    for (Config::PluginManifest &manifest : config.manifests) {
        ...
        EXPECT_STREQ(pluginDef.sharedLibraryPath.c_str(), "libPluginCommsTwoSixStub.so");
        EXPECT_STREQ(pluginDef.pythonModule.c_str(), "");
        EXPECT_STREQ(pluginDef.pythonClass.c_str(), "");

        const char *expectedPluginChannels[] = {"twoSixBootstrapCpp", "twoSixDirectCpp",
                                                "twoSixIndirectCpp", "twoSixIndirectBootstrapCpp"};
        EXPECT_EQ(pluginDef.channels.size(), 4);
        for (size_t ix = 0; ix < 4; ix++) {
            EXPECT_STREQ(pluginDef.channels.at(ix).c_str(), expectedPluginChannels[ix]);
        }
        ...
        EXPECT_STREQ(pluginDef.pythonModule.c_str(), "PluginDecomposedConjecture.doStuff");
        ...
        // verify channelGid
        const char *channelGids[] = {"twoSixBootstrapCpp", "twoSixDirectCpp",
                                     "twoSixIndirectBootstrapCpp", "twoSixIndirectComposition",
                                     "twoSixIndirectCpp"};
        size_t ix = 0;
        ...
        ChannelProperties &props = manifest.channelIdChannelPropsMap[channelGids[1]];
        ...
        std::vector<std::string> expectedKeys = {
            "hostname" + pluginNums[manifestIx],  "env" + pluginNums[manifestIx],
            "startPort" + pluginNums[manifestIx], "endPort" + pluginNums[manifestIx],
            "intVal" + pluginNums[manifestIx],    "boolVal" + pluginNums[manifestIx],
            "floatVal" + pluginNums[manifestIx],  "listSelectVal" + pluginNums[manifestIx],
        };

        // hostname
        size_t keyIx = 0;
        EXPECT_STREQ(expectedKeys[keyIx].c_str(), manifest.channelParameters[keyIx].key.c_str());
        EXPECT_TRUE(manifest.channelParameters[keyIx].required);
        EXPECT_EQ(manifest.channelParameters[keyIx].valueType, Config::ChannelParameter::STRING);
        ...
        EXPECT_STREQ(expectedKeys[keyIx].c_str(), manifest.channelParameters[keyIx].key.c_str());
        EXPECT_FALSE(manifest.channelParameters[keyIx].required);
        EXPECT_STREQ("12345", manifest.channelParameters[keyIx].defaultValue.c_str());
        EXPECT_EQ(manifest.channelParameters[keyIx].valueType, Config::ChannelParameter::INTEGER);
        ...
        EXPECT_STREQ(expectedKeys[keyIx].c_str(), manifest.channelParameters[keyIx].key.c_str());
        EXPECT_FALSE(manifest.channelParameters[keyIx].required);
        EXPECT_EQ(manifest.channelParameters[keyIx].valueType, Config::ChannelParameter::BOOLEAN);
        EXPECT_STREQ(manifest.channelParameters[keyIx].defaultValue.c_str(), "0");
        EXPECT_STREQ(manifest.channelParameters[keyIx].plugin.c_str(), pluginName.c_str());
