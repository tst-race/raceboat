...
#include "decomposed/ComponentActionManager.h"
...
public:
  ...
  TestableComponentActionManager(ComponentManagerInternal &cm)
      : ComponentActionManager(cm) {
    maxEncodingTime = 0.1;
  }

  bool testActionThreadLogic(Timestamp now) { return actionThreadLogic(now); }

  void testUpdateActionTimestamp() { updateActionTimestamp(); }

  void testUpdateEncodeTimestamp() { updateEncodeTimestamp(); }

protected:
  double currentTime() override { return nextTime; }
...
public:
  ComponentActionManagerTestFixture()
      : test_info(testing::UnitTest::GetInstance()->current_test_info()),
        ...
        mockComponentManager(logger), actionManager(mockComponentManager) {}
...
TEST_F(ComponentActionManagerTestFixture,
       test_updateGlobalTimeline_keep_actions_before_start) {
...
TEST_F(ComponentActionManagerTestFixture,
       test_updateGlobalTimeline_delete_actions_after_end) {
  actionManager.actions.push_back(std::make_unique<CMTypes::ActionInfo>(
...
TEST_F(ComponentActionManagerTestFixture,
       test_updateGlobalTimeline_add_new_actions_after_end) {
...
TEST_F(ComponentActionManagerTestFixture,
       test_updateGlobalTimeline_keep_action_in_both) {
  actionManager.actions.push_back(std::make_unique<CMTypes::ActionInfo>(
      CMTypes::ActionInfo{{2.0, 0, {}}, false, "", {}, {}, false}));
  ActionTimeline newActions = {{2.0, 0, {}}};
  LOG_EXPECT(this->logger, __func__, actionManager);
...
TEST_F(ComponentActionManagerTestFixture,
       test_updateGlobalTimeline_add_action_in_middle) {
  actionManager.actions.push_back(std::make_unique<CMTypes::ActionInfo>(
...
TEST_F(ComponentActionManagerTestFixture,
       test_updateGlobalTimeline_remove_action_in_middle) {
  actionManager.actions.push_back(std::make_unique<CMTypes::ActionInfo>(
...
TEST_F(ComponentActionManagerTestFixture, test_updateTimeline) {
  ...
  EXPECT_CALL(mockComponentManager.transport,
              getActionParams(Field(&Action::actionId, 1)))
      .WillOnce(Return(std::vector<EncodingParameters>{
          EncodingParameters{"mockLinkId", "*/*", true, "{}"}}));
  EXPECT_CALL(mockComponentManager.transport,
              getActionParams(Field(&Action::actionId, 2)))
      .WillOnce(Return(std::vector<EncodingParameters>{
          EncodingParameters{"*", "*/*", true, "{}"}}));
  EXPECT_CALL(mockComponentManager.transport,
              getActionParams(Field(&Action::actionId, 3)))
      .WillOnce(Return(std::vector<EncodingParameters>{
          EncodingParameters{"mockLinkId", "*/*", true, "{}"}}));
  LOG_EXPECT(this->logger, __func__, actionManager);
...
TEST_F(ComponentActionManagerTestFixture, test_actionThreadLogic_fetch) {
  EXPECT_CALL(mockComponentManager, getState())
      .WillOnce(Return(CMTypes::State::ACTIVATED));
  LOG_EXPECT(this->logger, __func__, actionManager);
  LOG_EXPECT(this->logger, __func__, actionManager.testActionThreadLogic(1));
  LOG_EXPECT(this->logger, __func__, actionManager);
}
  ...
  EXPECT_CALL(mockComponentManager, getPackageHandlesForAction(_))
      .WillOnce([this](CMTypes::ActionInfo *info) {
        ...
        return std::vector<CMTypes::PackageFragmentHandle>{
            CMTypes::PackageFragmentHandle{5},
