...
public:
  ComponentManagerTestFixture()
      : test_info(testing::UnitTest::GetInstance()->current_test_info()),
        logger(test_info->test_suite_name(), test_info->name()), sdk(logger),
        composition("composition_id", "transport_id", "usermodel_id",
                    {"encoding_id"}),
        ...
        manager(sdk, composition, transportPlugin, usermodelPlugin,
                std::unordered_map<std::string, IComponentPlugin *>{
                    {"encoder", &encodingPlugin}}) {}
  ...
  void activateChannels() {
    ...
    manager.waitForCallbacks();
    transportPlugin.transport->sdk.updateState(
        ComponentState::COMPONENT_STATE_STARTED);
    usermodelPlugin.userModel->sdk.updateState(
        ComponentState::COMPONENT_STATE_STARTED);
    encodingPlugin.encoding->sdk.updateState(
        ComponentState::COMPONENT_STATE_STARTED);
    manager.waitForCallbacks();
  }
  ...
  LinkID createLink() {
    ...
    EXPECT_CALL(*transportPlugin.transport, createLink(_, _))
        .WillOnce([this, &newLinkId](RaceHandle handle, const LinkID &linkId) {
          ...
          transportPlugin.transport->sdk.onLinkStatusChanged(handle, linkId,
                                                             LINK_CREATED, {});
          newLinkId = linkId;
    ...
    EXPECT_CALL(*usermodelPlugin.userModel, addLink(_, _))
        .WillOnce(
            [this, &wait](const LinkID &linkId, const LinkParameters &params) {
  ...
  ConnectionID createConnection(LinkID linkId) {
    ...
    // There may be a future CONNECTION_CLOSED call. Allow it by setting the
    // base case to AnyNumber.
    ...
    manager.openConnection(44, LT_BIDI, linkId, "",
...
TEST_F(ComponentManagerTestFixture, test_constructor) { log_expect(__func__); }
...
TEST_F(ComponentManagerTestFixture, test_activate2) {
  ...
  EXPECT_CALL(transportPlugin, createTransport(_, _, _, _))
      .WillOnce([this](std::string name, ITransportSdk *sdk,
                       std::string roleName, PluginConfig pluginConfig) {
        LOG_EXPECT(this->logger, this->transportPlugin.id + ".createTransport",
                   name, roleName, pluginConfig);
        this->transportPlugin.transport =
            std::make_shared<MockTransport>(this->logger, *sdk);
        transportPlugin.transport->sdk.updateState(
            ComponentState::COMPONENT_STATE_STARTED);
        return transportPlugin.transport;
      });
  EXPECT_CALL(usermodelPlugin, createUserModel(_, _, _, _))
      .WillOnce([this](std::string name, IUserModelSdk *sdk,
                       std::string roleName, PluginConfig pluginConfig) {
        LOG_EXPECT(this->logger, this->usermodelPlugin.id + ".createUserModel",
                   name, roleName, pluginConfig);
        usermodelPlugin.userModel =
            std::make_shared<MockUserModel>(this->logger, *sdk);
        usermodelPlugin.userModel->sdk.updateState(
            ComponentState::COMPONENT_STATE_STARTED);
        return usermodelPlugin.userModel;
      });
  EXPECT_CALL(encodingPlugin, createEncoding(_, _, _, _))
      .WillOnce([this](std::string name, IEncodingSdk *sdk,
                       std::string roleName, PluginConfig pluginConfig) {
        LOG_EXPECT(this->logger, this->encodingPlugin.id + ".createEncoding",
                   name, roleName, pluginConfig);
        encodingPlugin.encoding =
            std::make_shared<MockEncoding>(this->logger, *sdk);
        encodingPlugin.encoding->sdk.updateState(
            ComponentState::COMPONENT_STATE_STARTED);
...
TEST_F(ComponentManagerTestFixture, test_send_package) {
  ...
  EXPECT_CALL(*usermodelPlugin.userModel, getTimeline(_, _))
      .WillOnce([this](Timestamp startTime, Timestamp endTime) {
        ...
        action.actionId = 1;
        action.json = "";
        // The action should not happen during this test. Schedule it for 5
        // minutes from now.
  ...
  log_expect(__func__);
  manager.sendPackage(45, connId, pkg, std::numeric_limits<int32_t>::min(), 0);
  log_expect(__func__);
}

TEST_F(ComponentManagerTestFixture, test_request_plugin_user_input) {
  // Don't use activateChannel() to be sure it still works while components are
  // starting up.
  manager.init({});
  manager.activateChannel(42, composition.id, "some role name");
  manager.waitForCallbacks();
  ...
  log_expect(__func__);
  auto response = transportPlugin.transport->sdk.requestPluginUserInput(
      "key", "prompt", true);
...
TEST_F(ComponentManagerTestFixture, test_on_receive_bad_param_type) {
  ...
  transportPlugin.transport->sdk.onReceive(
      linkId, params, {0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8});
  log_expect(__func__);
}
...
TEST_F(ComponentManagerTestFixture, test_on_receive) {
  ...
  EXPECT_CALL(*encodingPlugin.encoding, decodeBytes(_, _, _))
      .WillOnce([this, &handle](RaceHandle decodeHandle,
                                const EncodingParameters &params,
                                const std::vector<uint8_t> &bytes) {
        LOG_EXPECT(this->logger, "decodeBytes", decodeHandle, params,
                   bytes.size());
        handle = decodeHandle;
        return ComponentStatus::COMPONENT_OK;
      });
  ...
  transportPlugin.transport->sdk.onReceive(
      linkId, params, {0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8});
  log_expect(__func__);
  encodingPlugin.encoding->sdk.onBytesDecoded(
      handle, {0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9}, ENCODE_OK);
  log_expect(__func__);
}
