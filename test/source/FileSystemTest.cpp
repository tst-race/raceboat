...
public:
  FileSystemTestFixture()
      : pluginId("examplePlugin1"),
        fs(INPUT_FILE_DIR, std::make_unique<Storage>()), fileName("testFile"),
        testPath(fs::path("test") / "path"), fileContents("hello") {
...
TEST_F(FileSystemTestFixture, binaryFile) {
  ...
  EXPECT_TRUE(std::equal(binaryFileData.begin(), binaryFileData.end(),
                         readBytes.begin()));
...
TEST_F(FileSystemTestFixture, makePluginFilePath) {
  fs::path expectedPath =
      fs::path(INPUT_FILE_DIR) / "usr" / pluginId / testPath;
