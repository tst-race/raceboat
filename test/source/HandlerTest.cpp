...
//         std::make_optional(true);}); (void)success1; (void)queueSize1;
//         future1.wait(); future1.get();
//     });
...
TEST(HandlerTest, test_post_before_start) {
  // if the thread continues after the timeout happens, bad stuff could occur if
  // we just used an int (or called the tests inside the callback)
  auto value1 = std::make_shared<bool>(false);
...
TEST(HandlerTest, test_callback_does_not_block_post) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    auto [success1, queueSize1, future1] = handler.post("", 0, 0, [&] {
      ...
      lock1_2.unlock();
      cv1.notify_one();
      // wait for the posting thread to unlock the mutex, signaling that it has
      // posted
...
TEST(HandlerTest, test_stop_immediate_breaks_promises) {
  ...
  bool finished = run_with_timeout([=] {
    auto handler_ptr = std::make_shared<Handler>("test-handler", max_queue_size,
                                                 max_total_size);
    auto promise_ptr = std::make_shared<std::promise<void>>();
    ...
    auto [success1, queueSize1, future1] =
        handler_ptr->post("", 0, 0, [handler_ptr, promise_ptr] {
          promise_ptr->set_value();
          // this sleep is because stop_immediate must be called during the
          // callback, and won't finish until the callback ends so it can't
          // signal us either
...
TEST(HandlerTest, test_stop_completes_promises) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    auto [success1, queueSize1, future1] =
        handler_ptr->post("", 0, 0, [handler_ptr, promise_ptr] {
          promise_ptr->set_value();
          // this sleep is because stop must be called during the callback, and
...
TEST(HandlerTest, test_stop_does_not_block_post) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    auto [success1, queueSize1,
          future1] = handler_ptr->post("", 0, 0, [handler_ptr, promise_ptr] {
      promise_ptr->set_value();
      // this sleep is because stop must be called during the callback, and
      // won't finish until the callback ends so it can't signal us either
      while (handler_ptr->get_state() == Handler::State::STARTED) {
      ...
      // we are now STOPPING. trying to post should return immediately and have
      // a future with an error.
      auto [success2, queueSize2, future2] =
          handler_ptr->post("", 0, 0, [] { return std::make_optional(true); });
      (void)success2;
      (void)queueSize2;
...
TEST(HandlerTest, test_start_create_remove_stop) {
  bool finished = run_with_timeout([=] {
    ...
    // should have 2 queues still. as the queue shouldn't be removed until it's
    // been hit by the handler thread. not required, so not checking it
    handler.stop();
  });
...
TEST(HandlerTest, test_post_queue_full) {
  ...
  bool finished = run_with_timeout([=] {
    Handler handler("test-handler", max_queue_size, max_total_size);
    auto [success1, queueSize1, future1] =
        handler.post("", handler.max_queue_size + 1, 0,
                     [] { return std::make_optional(true); });
  ...
  bool finished = run_with_timeout([=] {
    ...
    *value1 = (success1 != Handler::PostStatus::OK);
    ...
    auto [success2, queueSize2, future2] = handler.post(
        "", handler.max_queue_size, 0, [] { return std::make_optional(true); });
...
TEST(HandlerTest, test_post_post_queue_not_full) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    *value1 = (success1 == Handler::PostStatus::OK);
    ...
    (void)future1;
    auto [success2, queueSize2, future2] =
        handler.post("1", handler.max_queue_size, 0,
                     [] { return std::make_optional(true); });
    *value2 = (success2 == Handler::PostStatus::OK);
    (void)queueSize2;
    (void)future2;
...
TEST(HandlerTest, test_blocked_queue_unfinished) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    auto [success, queueSize, future] = handler.post(
        "", 0, 0, []() -> std::optional<bool> { return std::nullopt; });
    (void)success;
    (void)queueSize;
...
TEST(HandlerTest, test_blocked_queue_unblock) {
  bool finished = run_with_timeout([=] {
    ...
    handler.start();
    auto [success, queueSize, future] =
        handler.post("", 0, 0, [&]() -> std::optional<bool> {
          if (!count++) {
            promise_ptr->set_value();
            return std::nullopt;
          } else {
...
TEST(HandlerTest, test_blocked_queue_remove) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    handler.create_queue("1", 0);
    auto [success, queueSize, future] =
        handler.post("1", 0, 0, [&]() -> std::optional<bool> {
          if (!count++) {
            promise_ptr->set_value();
            return std::nullopt;
          } else {
...
TEST(HandlerTest, test_unblock_invalid_queue) {
  bool finished = run_with_timeout([=] {
    ...
    // This is expected not to throw an error, and any internal error should be
    // handled
...
TEST(HandlerTest, test_post_queue_full_with_post_timeout) {
  ...
  bool finished = run_with_timeout([=] {
    Handler handler("test-handler", max_queue_size, max_total_size);
    auto [success1, queueSize1, future1] =
        handler.post("", handler.max_queue_size + 1, 1,
                     [] { return std::make_optional(true); });
    *value1 = (success1 != Handler::PostStatus::OK);
    (void)queueSize1;
    (void)future1;
...
TEST(HandlerTest, test_post_queue_full_post_with_post_timeout) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    *value1 = (success1 != Handler::PostStatus::OK);
    ...
    (void)future1;
    auto [success2, queueSize2, future2] = handler.post(
        "", handler.max_queue_size, 1, [] { return std::make_optional(true); });
    *value2 = (success2 == Handler::PostStatus::OK);
    (void)queueSize2;
    (void)future2;
    handler.start();
...
TEST(HandlerTest, test_post_post_queue_full_post_timeout_success) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    handler.start();
    auto [success1, queueSize1, future1] =
        handler.post("", handler.max_queue_size, 1, [] {
          // this sleep is because the callback must finish while post is
          // waiting for its timeout, so it can't signal us
...
TEST(HandlerTest, test_post_post_race_blocking_post_timeout_blocks) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    *value1 = (success1 == Handler::PostStatus::OK);
    ...
    // expect run_with_timeout to timeout after 10 ms, before the post has
    // completed. The post should finish after 20ms, once the for callback above
    // has completed.
...
TEST(HandlerTest, test_post_blocking_too_large) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    handler.start();
    auto [success1, queueSize1, future1] =
        handler.post("", handler.max_queue_size + 1, RACE_BLOCKING,
                     [] { return std::make_optional(true); });
    *value1 = (success1 != Handler::PostStatus::OK);
    (void)queueSize1;
    (void)future1;
...
TEST(HandlerTest, test_post_post_handler_full_with_post_timeout) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    auto [success1, queueSize1, future1] =
        handler.post("1", handler.max_queue_size, 1,
                     [] { return std::make_optional(true); });
    *value1 = (success1 == Handler::PostStatus::OK);
    (void)queueSize1;
    (void)future1;
...
TEST(HandlerTest, test_post_expired_before_start_work_timeout_timedout) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    // timeout immediately
    auto [success, queueSize, future] = handler.post(
        "1", 0, 0, [] { return std::make_optional(true); }, 0.0,
        [value1] { *value1 = true; });
...
TEST(HandlerTest, test_post_before_start_work_timeout_timedout) {
  ...
  bool finished = run_with_timeout([=] {
    ...
    auto [success1, queueSize1, future1] =
        handler.post("1", 0, 0, [promise_ptr] {
