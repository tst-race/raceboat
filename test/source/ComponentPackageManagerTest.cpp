...
public:
  ComponentPackageManagerTestFixtureBase()
      : test_info(testing::UnitTest::GetInstance()->current_test_info()),
        ...
        mockComponentManager(logger), packageManager(mockComponentManager) {}

  virtual void TearDown() override { logger.check(); }

  CMTypes::PackageInfo *
  pushPackageOntoQueue(EncPkg &&pkg, CMTypes::PackageSdkHandle sdkHandle,
                       CMTypes::EncodingHandle encodingHandle,
                       CMTypes::Link *link = nullptr) {
    link = (link != nullptr) ? link : &mockComponentManager.mockLink;
    link->packageQueue.push_back(
        std::make_unique<CMTypes::PackageInfo>(CMTypes::PackageInfo{
            link, std::move(pkg), sdkHandle, encodingHandle, {}}));
    return link->packageQueue.back().get();
  }

  CMTypes::PackageFragmentInfo *
  createPackageFragment(CMTypes::PackageInfo *packageInfo,
                        CMTypes::ActionInfo *actionInfo,
                        CMTypes::PackageFragmentState state =
                            CMTypes::PackageFragmentState::UNENCODED) {
...
class ComponentPackageManagerTestFixture
    : public ComponentPackageManagerTestFixtureBase {
public:
...
TEST_P(ComponentPackageManagerTestFixture, test_sendPackage_no_timeline) {
  ...
  EXPECT_EQ(PLUGIN_TEMP_ERROR,
            packageManager.sendPackage({0}, 1.0, {7}, "mockConnectionId",
...
TEST_P(ComponentPackageManagerTestFixture,
       test_sendPackage_no_available_actions) {
  ...
  mockAction1.encoding.push_back(
      {{}, {1000}, {0}, CMTypes::EncodingState::ENCODING, nullptr});
  ...
  mockAction2.encoding.push_back(
      {{}, {1000}, {0}, CMTypes::EncodingState::UNENCODED, nullptr});
  ...
  LOG_EXPECT(this->logger, __func__, mockComponentManager.mockLink);
  EXPECT_EQ(PLUGIN_TEMP_ERROR,
            packageManager.sendPackage({0}, now, {7}, "mockConnectionId",
                                       EncPkg(1, 2, {0x12, 0x34}), 0.0, 0));
  LOG_EXPECT(this->logger, __func__, packageManager);
  LOG_EXPECT(this->logger, __func__, mockComponentManager.mockLink);
}
...
TEST_P(ComponentPackageManagerTestFixture,
       test_sendPackage_available_action_explicit_link_id) {
  double now = helper::currentTime();
  CMTypes::ActionInfo mockAction;
...
TEST_P(ComponentPackageManagerTestFixture,
       test_sendPackage_available_action_existing_fragment) {
  double now = helper::currentTime();
  ...
  mockAction.encoding.push_back(
      {{}, {1000}, {0}, CMTypes::EncodingState::UNENCODED, nullptr});
  createPackageFragment(package, &mockAction,
                        CMTypes::PackageFragmentState::UNENCODED);
  mockAction.toBeRemoved = false;
  mockComponentManager.mockLink.actionQueue.push_back(&mockAction);
...
TEST_P(ComponentPackageManagerTestFixture,
  ...
  mockAction1.action.timestamp = now + 100;
  mockAction1.encoding.push_back(
      {{}, {100}, {0}, CMTypes::EncodingState::UNENCODED, nullptr});
  mockAction1.toBeRemoved = false;
  mockComponentManager.mockLink.actionQueue.push_back(&mockAction1);
  ...
  mockAction3.action.actionId = 5;
  mockAction3.action.timestamp = now + 100;
  mockAction3.encoding.push_back(
      {{}, {100}, {0}, CMTypes::EncodingState::UNENCODED, nullptr});
  mockAction3.toBeRemoved = false;
  mockComponentManager.mockLink.actionQueue.push_back(&mockAction3);
  ...
  LOG_EXPECT(this->logger, __func__, mockComponentManager.mockLink);
  auto resp = packageManager.sendPackage({0}, now, {7}, "mockConnectionId",
                                         EncPkg(std::vector<uint8_t>(220, 'a')),
                                         0.0, 0);
  LOG_EXPECT(this->logger, __func__, resp);
  LOG_EXPECT(this->logger, __func__, packageManager);
  LOG_EXPECT(this->logger, __func__, mockComponentManager.mockLink);
...
TEST_P(ComponentPackageManagerTestFixture,
       test_encodeCoverTrafficForAction_no_encodings) {
  double now = helper::currentTime();
  CMTypes::ActionInfo mockAction;
  mockAction.action.actionId = 3;
  mockAction.action.timestamp = now + 100;
...
TEST_P(ComponentPackageManagerTestFixture,
       test_encodeCoverTrafficForAction_no_packages) {
  double now = helper::currentTime();
  CMTypes::ActionInfo mockAction;
  mockAction.action.actionId = 3;
  mockAction.action.timestamp = now + 100;
...
TEST_P(ComponentPackageManagerTestFixture,
       test_encodeCoverTrafficForAction_single_packages) {
  double now = helper::currentTime();
  auto package = pushPackageOntoQueue(EncPkg(1, 2, {0x12, 0x34}), {6}, {3});
...
TEST_P(ComponentPackageManagerTestFixture,
       test_encodeCoverTrafficForAction_multiple_packages) {
  double now = helper::currentTime();
  ...
  mockAction.encoding.push_back(
      {{}, {1000}, {0}, CMTypes::EncodingState::UNENCODED, nullptr});
  createPackageFragment(package1, &mockAction,
                        CMTypes::PackageFragmentState::UNENCODED);
  createPackageFragment(package2, &mockAction,
                        CMTypes::PackageFragmentState::UNENCODED);
  mockAction.toBeRemoved = false;
  mockComponentManager.mockLink.actionQueue.push_back(&mockAction);
  ...
  if (mockComponentManager.mode == CMTypes::EncodingMode::SINGLE) {
    EXPECT_THROW(packageManager.encodeForAction(&mockAction),
                 std::runtime_error);
  } else {
    packageManager.encodeForAction(&mockAction);
  }
...
TEST_P(ComponentPackageManagerTestFixture,
       test_encodeCoverTrafficForAction_fragmented_packages) {
  double now = helper::currentTime();
  auto package1 = pushPackageOntoQueue(
      EncPkg(1, 2, std::vector<uint8_t>(300, 'a')), {6}, {3});
  CMTypes::ActionInfo mockAction;
  mockAction.action.actionId = 3;
  mockAction.action.timestamp = now + 100;
  mockAction.linkId = "mockLinkId";
...
TEST_P(ComponentPackageManagerTestFixtureNonParameterized,
  ...
  auto package2 =
      pushPackageOntoQueue(EncPkg(2, 3, {0x31, 0x41, 0x59}), {8}, {4});

  CMTypes::ActionInfo mockAction1;
  mockAction1.action.actionId = 42;
  mockAction1.action.timestamp = now + 100;
  mockAction1.encoding.push_back(
      {{}, {1000}, {3}, CMTypes::EncodingState::ENQUEUED, nullptr});
  createPackageFragment(package1, &mockAction1,
                        CMTypes::PackageFragmentState::ENQUEUED);
  mockAction1.toBeRemoved = false;
  mockComponentManager.mockLink.actionQueue.push_back(&mockAction1);
  ...
  mockAction2.action.timestamp = now + 100;
  mockAction2.encoding.push_back(
      {{}, {1000}, {4}, CMTypes::EncodingState::ENCODING, nullptr});
  createPackageFragment(package2, &mockAction2,
                        CMTypes::PackageFragmentState::ENCODING);
  mockAction2.toBeRemoved = false;
  mockComponentManager.mockLink.actionQueue.push_back(&mockAction2);
  ...
  packageManager.onLinkStatusChanged(
      {3}, {8}, mockComponentManager.mockLink.linkId, LINK_DESTROYED, {});
...
TEST_P(ComponentPackageManagerTestFixtureNonParameterized,
  ...
  EXPECT_EQ(CMTypes::CmInternalStatus::OK,
            packageManager.onBytesEncoded({1}, {42}, {0x12, 0x34},
                                          EncodingStatus::ENCODE_OK));
  LOG_EXPECT(this->logger, __func__, packageManager);
}

TEST_P(ComponentPackageManagerTestFixtureNonParameterized,
       test_onBytesEncoded_failed_encoding) {
  CMTypes::EncodingInfo encodingInfo{
      {}, {1000}, {42}, CMTypes::EncodingState::ENCODING, nullptr};
  packageManager.pendingEncodings[{42}] = &encodingInfo;
  LOG_EXPECT(this->logger, __func__, packageManager);
  EXPECT_EQ(CMTypes::CmInternalStatus::OK,
            packageManager.onBytesEncoded({1}, {42}, {},
                                          EncodingStatus::ENCODE_FAILED));
  LOG_EXPECT(this->logger, __func__, packageManager);
}
...
TEST_P(ComponentPackageManagerTestFixture,
       test_onPackageStatusChanged_no_package_found) {
  EXPECT_EQ(CMTypes::CmInternalStatus::OK,
            packageManager.onPackageStatusChanged({9}, {7},
...
TEST_P(ComponentPackageManagerTestFixture,
       test_onPackageStatusChanged_valid_package) {
  auto package1 = pushPackageOntoQueue(EncPkg(1, 2, {0x12, 0x34}), {7}, {3});

  auto frag1 = createPackageFragment(package1, nullptr,
                                     CMTypes::PackageFragmentState::DONE);
...
TEST_P(ComponentPackageManagerTestFixture,
       test_onPackageStatusChanged_package_fragment_not_done) {
  auto package1 = pushPackageOntoQueue(EncPkg(1, 2, {0x12, 0x34}), {7}, {3});

  auto frag1 = createPackageFragment(package1, nullptr,
                                     CMTypes::PackageFragmentState::DONE);
  auto frag2 = createPackageFragment(package1, nullptr,
                                     CMTypes::PackageFragmentState::ENQUEUED);
  frag1->len /= 2;
...
TEST_P(ComponentPackageManagerTestFixture,
  ...
  LOG_EXPECT(this->logger, __func__, mockComponentManager.mockLink);
  EXPECT_EQ(CMTypes::CmInternalStatus::OK,
            packageManager.onPackageStatusChanged(
                {9}, package1->packageFragments.front()->handle,
                PackageStatus::PACKAGE_SENT));
  LOG_EXPECT(this->logger, __func__, packageManager);
  LOG_EXPECT(this->logger, __func__, mockComponentManager.mockLink);
}
...
TEST_P(ComponentPackageManagerTestFixture,
  ...
  auto frag1 = createPackageFragment(package1, nullptr,
                                     CMTypes::PackageFragmentState::SENT);
  auto frag2 = createPackageFragment(package1, nullptr,
                                     CMTypes::PackageFragmentState::DONE);
...
TEST_P(ComponentPackageManagerTestFixture,
  ...
  // the fragments shuold be destroyed after the first call, so get the handles
  // now
  ...
  EXPECT_EQ(CMTypes::CmInternalStatus::OK,
            packageManager.onPackageStatusChanged(
                {9}, fragHandle1, PackageStatus::PACKAGE_FAILED_GENERIC));
  EXPECT_EQ(CMTypes::CmInternalStatus::OK,
            packageManager.onPackageStatusChanged({9}, fragHandle2,
                                                  PackageStatus::PACKAGE_SENT));
  LOG_EXPECT(this->logger, __func__, packageManager);
  LOG_EXPECT(this->logger, __func__, mockComponentManager.mockLink);
}
...
TEST_P(ComponentPackageManagerTestFixture,
  ...
  EXPECT_EQ(CMTypes::CmInternalStatus::OK,
            packageManager.onPackageStatusChanged(
                {9}, frag2->handle, PackageStatus::PACKAGE_FAILED_GENERIC));
  LOG_EXPECT(this->logger, __func__, packageManager);
  LOG_EXPECT(this->logger, __func__, mockComponentManager.mockLink);
}
...
TEST_P(
  ...
  mockAction1.action.timestamp = now + 100;
  mockAction1.encoding.push_back(
      {{}, {1000}, {3}, CMTypes::EncodingState::UNENCODED, nullptr});
  mockAction1.toBeRemoved = false;
  mockComponentManager.mockLink.actionQueue.push_back(&mockAction1);

  auto frag1 = createPackageFragment(package1, nullptr,
                                     CMTypes::PackageFragmentState::DONE);
  auto frag2 = createPackageFragment(package1, &mockAction1,
                                     CMTypes::PackageFragmentState::UNENCODED);
  frag1->len /= 2;
  frag2->offset = frag1->len;
  frag2->len = frag2->len - frag2->offset;
  ...
  EXPECT_EQ(CMTypes::CmInternalStatus::OK,
            packageManager.onPackageStatusChanged(
                {9}, frag1->handle, PackageStatus::PACKAGE_FAILED_GENERIC));
  LOG_EXPECT(this->logger, __func__, packageManager);
  LOG_EXPECT(this->logger, __func__, mockComponentManager.mockLink);
}
...
TEST_P(
  ...
  frag2->offset = frag1->len;
  frag2->len = frag2->len - frag2->offset;
  auto frag3 = createPackageFragment(package2, &mockAction1,
                                     CMTypes::PackageFragmentState::UNENCODED);
  auto frag4 = createPackageFragment(package2, &mockAction2,
                                     CMTypes::PackageFragmentState::UNENCODED);
  frag3->len /= 2;
...
TEST_P(
  ...
  auto package1 = pushPackageOntoQueue(EncPkg(1, 2, {0x12, 0x34}), {7}, {0});
  auto package2 = pushPackageOntoQueue(EncPkg(3, 4, {0x12, 0x34}), {8}, {0});
  pushPackageOntoQueue(EncPkg(5, 6, {0x12, 0x34}), {9}, {0},
                       &mockComponentManager.mockLink2);

  CMTypes::ActionInfo mockAction1;
  mockAction1.action.actionId = 42;
  mockAction1.action.timestamp = now + 100;
  mockAction1.encoding.push_back({{"mockLinkId", "*/*", true, ""},
                                  {1000},
                                  {0},
                                  CMTypes::EncodingState::UNENCODED,
                                  nullptr});
  ...
  mockAction2.action.actionId = 43;
  mockAction2.action.timestamp = now + 100;
  mockAction2.encoding.push_back({{"mockLinkId", "*/*", true, ""},
                                  {1000},
                                  {0},
                                  CMTypes::EncodingState::UNENCODED,
                                  nullptr});
...
TEST_P(ComponentPackageManagerTestFixture,
  ...
  double now = helper::currentTime();
  auto package1 = pushPackageOntoQueue(
      EncPkg(1, 2, std::vector<uint8_t>(100, 'a')), {7}, {3});
  auto package2 = pushPackageOntoQueue(
      EncPkg(3, 4, std::vector<uint8_t>(100, 'b')), {8}, {4});
  auto package3 = pushPackageOntoQueue(
      EncPkg(5, 6, std::vector<uint8_t>(100, 'c')), {9}, {5});
  auto package4 = pushPackageOntoQueue(
      EncPkg(7, 8, std::vector<uint8_t>(100, 'd')), {10}, {6});
  auto package5 = pushPackageOntoQueue(
      EncPkg(9, 10, std::vector<uint8_t>(100, 'e')), {11}, {7});
...
TEST_P(ComponentPackageManagerTestFixtureNonParameterized, test_actionDone) {
  ...
  createPackageFragment(package1, &mockAction1,
                        CMTypes::PackageFragmentState::ENQUEUED);
  auto frag1 = createPackageFragment(package2, &mockAction1,
                                     CMTypes::PackageFragmentState::ENQUEUED);
  auto frag2 = createPackageFragment(package2, &mockAction2,
                                     CMTypes::PackageFragmentState::ENQUEUED);
  frag1->len /= 2;
  frag2->offset = frag1->len;
  frag2->len = frag2->len - frag2->offset;
...
TEST_P(ComponentPackageManagerTestFixtureNonParameterized,
       test_getPackageHandlesForAction) {
  double now = helper::currentTime();
  CMTypes::PackageInfo mockPackage1{
      nullptr, EncPkg(1, 2, {0x12, 0x34}), {5}, {2}, {}};
  CMTypes::PackageInfo mockPackage2{
      nullptr, EncPkg(1, 2, {0x12, 0x34}), {7}, {3}, {}};
...
TEST_P(ComponentPackageManagerTestFixtureNonParameterized, test_teardown) {
  ...
  CMTypes::PackageInfo mockPackage2{&mockComponentManager.mockLink,
                                    EncPkg(2, 3, {0x31, 0x41, 0x59}),
                                    {7},
                                    {3},
                                    {}};
  ...
  CMTypes::EncodingInfo encodingInfo{
      {}, {1000}, {3}, CMTypes::EncodingState::ENCODING, nullptr};
...
INSTANTIATE_TEST_SUITE_P(
    ComponentPackageManagerTestSuite, ComponentPackageManagerTestFixture,
    testing::Values(CMTypes::EncodingMode::SINGLE, CMTypes::EncodingMode::BATCH,
