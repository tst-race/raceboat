...
TEST(PluginWrapperTest, init) {
  ...
  EXPECT_CALL(*mockPlugin, init(pluginConfig))
      .Times(1)
...
TEST(PluginWrapperTest, sendPackage) {
  ...
  EXPECT_CALL(*mockPlugin,
              sendPackage(handle, connId, sentPkg,
...
TEST(PluginWrapperTest, openConnection) {
  ...
  EXPECT_CALL(*mockPlugin,
              openConnection(handle, linkType, linkId, linkHints, 0))
      .WillOnce(Return(PLUGIN_OK));
...
TEST(PluginWrapperTest, activateChannel) {
  ...
  EXPECT_CALL(*mockPlugin, activateChannel(handle, channelGid, roleName))
      .Times(1);
...
TEST(PluginWrapperTest, loadLinkAddress) {
  ...
  EXPECT_CALL(*mockPlugin, loadLinkAddress(handle, channelGid, linkAddress))
      .Times(1);
...
TEST(PluginWrapperTest, loadLinkAddressses) {
  ...
  EXPECT_CALL(*mockPlugin, loadLinkAddresses(handle, channelGid, linkAddresses))
      .Times(1);
...
TEST(PluginWrapperTest, createLinkFromAddress) {
  ...
  EXPECT_CALL(*mockPlugin,
              createLinkFromAddress(handle, channelGid, linkAddress))
      .Times(1);
...
TEST(PluginWrapperTest, onUserInputReceived) {
  ...
  EXPECT_CALL(*mockPlugin,
              onUserInputReceived(0x11223344l, true, "expected-response"))
      .Times(1);
