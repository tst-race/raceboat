...
TEST(PluginLoading, test_unified) {
  ...
  EXPECT_CALL(core.mockFS,
              makePluginInstallPath(fs::path("libunified-test-stub.so"),
                                    "UnifiedTestStub"))
      .WillOnce(Return(stub_path));
  EXPECT_CALL(core.mockFS,
              makePluginInstallPath(fs::path(""), "UnifiedTestStub"))
...
TEST(PluginLoading, test_decomposed) {
  ...
  auto core = MockCore(plugin_loader_path, config);
  EXPECT_CALL(core.mockFS,
              makePluginInstallPath(fs::path(""), "DecomposedTestStub"))
      .WillOnce(Return(transportPath.parent_path() / ""));
  EXPECT_CALL(core.mockFS,
              makePluginInstallPath(fs::path("libDecomposedTestTransport.so"),
                                    "DecomposedTestStub"))
      .WillOnce(Return(transportPath.parent_path() /
                       "libDecomposedTestTransport.so"));
  EXPECT_CALL(core.mockFS,
              makePluginInstallPath(fs::path("libDecomposedTestUserModel.so"),
                                    "DecomposedTestStub"))
      .WillOnce(Return(transportPath.parent_path() /
                       "libDecomposedTestUserModel.so"));
  EXPECT_CALL(core.mockFS,
              makePluginInstallPath(fs::path("libDecomposedTestEncoding.so"),
                                    "DecomposedTestStub"))
      .WillOnce(
          Return(transportPath.parent_path() / "libDecomposedTestEncoding.so"));
  ...
  auto response = container1->plugin->activateChannel(42, "DecomposedTestStub",
                                                      "default", 0);
