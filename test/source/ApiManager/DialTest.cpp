...
public:
    ApiManagerDialTestFixture() : params(), core(std::make_shared<MockCore>()), manager(*core) {
...
void ApiManagerDialTestFixture::expectActivateSendChannel(RaceHandle &handle,
...
void ApiManagerDialTestFixture::expectActivateRecvChannel(RaceHandle &handle,
                                                          ActivateChannelStatusCode thisStatus) {
    EXPECT_CALL(core->mockChannelManager,
...
void ApiManagerDialTestFixture::expectLoadLinkAddress(RaceHandle &handle,
                                                      std::string send_address) {
...
void ApiManagerDialTestFixture::expectOpenConnection(RaceHandle &handle, ConnectionID thisConnId) {
...
void ApiManagerDialTestFixture::expectSendPackage(RaceHandle &handle, ConnectionID thisConnId) {
...
void ApiManagerDialTestFixture::expectCloseConnection(RaceHandle &handle, ConnectionID thisConnId) {
...
void ApiManagerDialTestFixture::expectDestroyLink(RaceHandle &handle, LinkID thisLinkId) {
...
void ApiManagerDialTestFixture::dialCall(std::function<void(ApiStatus, RaceHandle)> callback,
                                         std::string send_address) {
...
void ApiManagerDialTestFixture::writeCall(RaceHandle handle,
                                          std::function<void(ApiStatus)> callback) {
    manager.write(handle, sendBytes, callback);
    manager.waitForCallbacks();
}

void ApiManagerDialTestFixture::closeCall(RaceHandle handle,
                                          std::function<void(ApiStatus)> callback) {
    manager.close(handle, callback);
    manager.waitForCallbacks();
}

void ApiManagerDialTestFixture::onChannelStatusChangedCall(RaceHandle handle, ChannelId channelId) {
    manager.onChannelStatusChanged(*core->container, handle, channelId, CHANNEL_AVAILABLE,
                                   channelProps);
    manager.waitForCallbacks();
}

void ApiManagerDialTestFixture::onLinkStatusChangedCall(RaceHandle handle, LinkID thisLinkId) {
    manager.onLinkStatusChanged(*core->container, handle, thisLinkId, LINK_LOADED, linkProps);
    manager.waitForCallbacks();
}

void ApiManagerDialTestFixture::onConnectionStatusChangedCall(RaceHandle handle,
...
void ApiManagerDialTestFixture::onConnectionStatusChangedClosedCall(RaceHandle handle,
                                                                    ConnectionID thisConnId) {
...
void ApiManagerDialTestFixture::onLinkStatusChangedDestroyedCall(RaceHandle handle,
                                                                 LinkID thisLinkId) {
...
TEST_F(ApiManagerDialTestFixture, no_errors) {
    EXPECT_CALL(*core, getEntropy(_)).WillRepeatedly(Return(std::vector<uint8_t>(16, 1)));
...
TEST_F(ApiManagerDialTestFixture, write_fail) {
    ...
    writeCall(dialHandle, writeCallback);
    manager.onPackageStatusChanged(*core->container, pkgHandle2, PACKAGE_FAILED_GENERIC);
    manager.waitForCallbacks();
