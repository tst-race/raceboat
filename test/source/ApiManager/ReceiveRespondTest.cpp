...
public:
    ApiManagerReceiveRespondTestFixture() :
        ...
        getReceiverCallback = [this](ApiStatus _status, LinkAddress address, RaceHandle _handle) {
            status1 = _status;
            recvAddress = address;
    ...
    std::function<void(ApiStatus, std::vector<uint8_t>, LinkAddress)> receiveCallback;
...
void ApiManagerReceiveRespondTestFixture::expectLoadLinkAddress(RaceHandle &handle,
                                                                std::string _sendAddress) {
    // use member variable if argument is not specified
    _sendAddress = _sendAddress.empty() ? sendAddress : _sendAddress;
    EXPECT_CALL(*core->plugin, loadLinkAddress(_, recvOptions.send_channel, _sendAddress, _))
...
void ApiManagerReceiveRespondTestFixture::expectOpenConnection(RaceHandle &handle,
...
void ApiManagerReceiveRespondTestFixture::expectSendPackage(RaceHandle &handle,
                                                            ConnectionID thisConnId) {
...
void ApiManagerReceiveRespondTestFixture::expectCloseConnection(RaceHandle &handle,
                                                                ConnectionID thisConnId) {
...
void ApiManagerReceiveRespondTestFixture::expectDestroyLink(RaceHandle &handle, LinkID thisLinkId) {
...
void ApiManagerReceiveRespondTestFixture::receiveRespondCall(
    RaceHandle handle, std::function<void(ApiStatus, std::vector<uint8_t>, LinkAddress)> callback) {
    // ReceiveOptions recvOptionsCopy = recvOptions;
    // recvOptionsCopy.recv_address = _recvAddress.empty() ? recvOptionsCopy.recv_address :
    // _recvAddress;
    manager.receiveRespond(handle, callback);
    manager.waitForCallbacks();
}

void ApiManagerReceiveRespondTestFixture::sendCall(std::function<void(ApiStatus)> callback,
...
void ApiManagerReceiveRespondTestFixture::onChannelStatusChangedCall(RaceHandle handle,
                                                                     ChannelId channelId) {
...
void ApiManagerReceiveRespondTestFixture::onLinkStatusChangedCall(RaceHandle handle,
                                                                  LinkID thisLinkId) {
    manager.onLinkStatusChanged(*core->container, handle, thisLinkId, LINK_LOADED, linkProps);
    manager.waitForCallbacks();
}

void ApiManagerReceiveRespondTestFixture::onConnectionStatusChangedCall(RaceHandle handle,
                                                                        ConnectionID thisConnId) {
...
void ApiManagerReceiveRespondTestFixture::onPackageStatusChangedCall(RaceHandle handle) {
    manager.onPackageStatusChanged(*core->container, handle, PACKAGE_SENT);
    manager.waitForCallbacks();
}

void ApiManagerReceiveRespondTestFixture::receiveEncPkgCall(std::vector<uint8_t> &bytes,
                                                            ConnectionID thisConnId) {
...
void ApiManagerReceiveRespondTestFixture::closeCall(OpHandle handle,
...
void ApiManagerReceiveRespondTestFixture::onLinkStatusChangedDestroyedCall(RaceHandle handle,
                                                                           LinkID thisLinkId) {
