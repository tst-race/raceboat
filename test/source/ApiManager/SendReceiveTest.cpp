...
public:
    ApiManagerSendReceiveTestFixture() :
        ...
        sendReceiveCallback = [this](ApiStatus _status, std::vector<uint8_t> bytes) {
            status = _status;
            bytes2 = bytes;
        };
    }
    ...
    void sendReceiveCall(std::function<void(ApiStatus, std::vector<uint8_t>)> callback,
...
void ApiManagerSendReceiveTestFixture::expectLoadLinkAddress(RaceHandle &handle,
                                                             std::string send_address) {
    send_address = send_address.empty() ? sendOptions.send_address : send_address;
    EXPECT_CALL(*core->plugin, loadLinkAddress(_, sendOptions.send_channel, send_address, _))
...
void ApiManagerSendReceiveTestFixture::expectOpenConnection(RaceHandle &handle,
...
void ApiManagerSendReceiveTestFixture::expectSendPackage(RaceHandle &handle,
                                                         ConnectionID thisConnId) {
...
void ApiManagerSendReceiveTestFixture::expectCloseConnection(RaceHandle &handle,
                                                             ConnectionID thisConnId) {
...
void ApiManagerSendReceiveTestFixture::expectDestroyLink(RaceHandle &handle, LinkID thisLinkId) {
...
void ApiManagerSendReceiveTestFixture::sendReceiveCall(
    std::function<void(ApiStatus, std::vector<uint8_t>)> callback, std::string send_address) {
    SendOptions sendOptionsCopy = sendOptions;
    sendOptionsCopy.send_address = send_address.empty() ? sendOptions.send_address : send_address;
    SdkResponse response = manager.sendReceive(sendOptionsCopy, sendBytes, callback);
    EXPECT_EQ(response.status, SDK_OK);
    manager.waitForCallbacks();
}

void ApiManagerSendReceiveTestFixture::onChannelStatusChangedCall(RaceHandle handle,
                                                                  ChannelId channelId) {
    SdkResponse response = manager.onChannelStatusChanged(*core->container, handle, channelId,
                                                          CHANNEL_AVAILABLE, channelProps);
    EXPECT_EQ(response.status, SDK_OK);
    manager.waitForCallbacks();
}

void ApiManagerSendReceiveTestFixture::onLinkStatusChangedCall(RaceHandle handle,
                                                               LinkID thisLinkId) {
...
void ApiManagerSendReceiveTestFixture::onConnectionStatusChangedCall(RaceHandle handle,
                                                                     ConnectionID thisConnId) {
    SdkResponse response = manager.onConnectionStatusChanged(*core->container, handle, thisConnId,
                                                             CONNECTION_OPEN, linkProps);
    EXPECT_EQ(response.status, SDK_OK);
    manager.waitForCallbacks();
}

void ApiManagerSendReceiveTestFixture::onPackageStatusChangedCall(RaceHandle handle) {
    SdkResponse response = manager.onPackageStatusChanged(*core->container, handle, PACKAGE_SENT);
    EXPECT_EQ(response.status, SDK_OK);
    manager.waitForCallbacks();
}

void ApiManagerSendReceiveTestFixture::receiveEncPkgCall(std::vector<uint8_t> &bytes,
                                                         ConnectionID thisConnId) {
    EncPkg pkg(0, 0, bytes);
    SdkResponse response = manager.receiveEncPkg(*core->container, pkg, {thisConnId});
    EXPECT_EQ(response.status, SDK_OK);
    manager.waitForCallbacks();
}
...
void ApiManagerSendReceiveTestFixture::onLinkStatusChangedDestroyedCall(RaceHandle handle,
                                                                        LinkID thisLinkId) {
    SdkResponse response = manager.onLinkStatusChanged(*core->container, handle, thisLinkId,
...
TEST_F(ApiManagerSendReceiveTestFixture, no_errors) {
    ...
    expectActivateRecvChannel(chanHandle2, ActivateChannelStatusCode::ALREADY_ACTIVATED);
...
TEST_F(ApiManagerSendReceiveTestFixture, activateChannel_1_error) {
    expectActivateSendChannel(chanHandle, ActivateChannelStatusCode::CHANNEL_DOES_NOT_EXIST);
...
TEST_F(ApiManagerSendReceiveTestFixture, activateChannel_2_error) {
    ...
    RaceHandle chanHandle2;
    expectActivateRecvChannel(chanHandle2, ActivateChannelStatusCode::CHANNEL_DOES_NOT_EXIST);
...
TEST_F(ApiManagerSendReceiveTestFixture, onChannelStatusChanged_send_error) {
    ...
    manager.onChannelStatusChanged(*core->container, NULL_RACE_HANDLE, sendOptions.send_channel,
                                   CHANNEL_FAILED, {});
...
TEST_F(ApiManagerSendReceiveTestFixture, onChannelStatusChanged_recv_error) {
    ...
    onChannelStatusChangedCall(chanHandle, sendOptions.send_channel);
    manager.onChannelStatusChanged(*core->container, NULL_RACE_HANDLE, sendOptions.recv_channel,
                                   CHANNEL_FAILED, {});
...
TEST_F(ApiManagerSendReceiveTestFixture, loadLinkAddress_error) {
    ...
    EXPECT_CALL(*core->plugin,
                loadLinkAddress(_, sendOptions.send_channel, sendOptions.send_address, _))
        .WillOnce(Return(SdkResponse(SDK_INVALID_ARGUMENT)));
...
TEST_F(ApiManagerSendReceiveTestFixture, onLinkStatusChanged_recv_error) {
    ...
    onChannelStatusChangedCall(chanHandle, sendOptions.send_channel);
    onChannelStatusChangedCall(chanHandle2, sendOptions.recv_channel);
    manager.onLinkStatusChanged(*core->container, createLinkHandle, recvLinkId, LINK_DESTROYED,
                                linkProps);
    onLinkStatusChangedCall(loadLinkHandle, sendLinkId);
...
TEST_F(ApiManagerSendReceiveTestFixture, onLinkStatusChanged_send_error) {
    ...
    onChannelStatusChangedCall(chanHandle, sendOptions.send_channel);
    onChannelStatusChangedCall(chanHandle2, sendOptions.recv_channel);
    manager.onLinkStatusChanged(*core->container, loadLinkHandle, sendLinkId, LINK_DESTROYED,
                                linkProps);
    onLinkStatusChangedCall(createLinkHandle, recvLinkId);
...
TEST_F(ApiManagerSendReceiveTestFixture, onConnectionStatusChanged_open_recv_error) {
    expectActivateSendChannel(chanHandle);
...
TEST_F(ApiManagerSendReceiveTestFixture, onConnectionStatusChanged_open_send_error) {
    expectActivateSendChannel(chanHandle);
...
TEST_F(ApiManagerSendReceiveTestFixture, onPackageStatusChanged_error) {
    ...
    onConnectionStatusChangedCall(openSendConnHandle, sendConnId);
    onConnectionStatusChangedCall(openRecvConnHandle, recvConnId);
    manager.onPackageStatusChanged(*core->container, pkgHandle, PACKAGE_FAILED_GENERIC);
