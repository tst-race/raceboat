...
TEST(ApiTest, oneshot) {
    ...
    auto thread1 = std::thread([promise = std::move(linkAddrPromise)]() mutable {
        ...
        // An alternative is to store the required keys at the channel level in the manifest instead
        // of at the plugin level. This fixes the issue with the user not knowing exactly which keys
        // are required for a specific channel, but has other issues. If multiple channels in a
        // single plugin require the same key, and the user specifies the values at a per channel
        // level, one value could overwrite another. Additionally, we would have to find out which
        // keys are required per channel for existing RACE channels, and can't just create a script
        // to do it.
        ...
        EXPECT_CALL(core->mockApiManager, getReceiveObject(_, _)).WillOnce([](auto, auto callback) {
        ...
        EXPECT_CALL(core->mockApiManager, receive(_, _)).WillOnce([](auto, auto callback) {
            std::string message = "Hello, World!";
            callback(ApiStatus::OK, std::vector<uint8_t>{message.begin(), message.end()});
            return SDK_OK;
        });
    ...
    auto thread2 = std::thread([future = std::move(future)]() mutable {
        ...
        EXPECT_CALL(core->mockApiManager, send(_, _, _))
            .Times(1)
            .WillOnce(
                [](SendOptions, std::vector<uint8_t>, std::function<void(ApiStatus)> callback) {
                    callback(ApiStatus::OK);
                    return SDK_OK;
                });
...
TEST(ApiTest, DISABLED_bidi_channel) {
    ...
    auto thread1 = std::thread([promise = std::move(linkAddrPromise)]() mutable {
        ...
        // TODO: is there a sensible way to accept with a response? Should the accept call take an
        // argument? If it did, that message could not depend on the received message. Otherwise, it
        // would be possible to return an reply object that could be used for the reply. That might
...
TEST(ApiTest, DISABLED_async_bidi_oneshot) {
    ...
    auto thread1 = std::thread([promise = std::move(linkAddrPromise)]() mutable {
        ...
        race.receive_respond(opt, [promise = std::move(promise)](const LinkAddress addr,
RespondObject listener) mutable { promise.set_value(addr);
