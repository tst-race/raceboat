...
class BuildTestOrchestrator:
    ...
    def _validate_plugin_structure(self):
        ...
        # Plugins own only their test/adapter.py (scenario-driven, see
        # raceboat/test/integration/generate_scenario.py); legacy plugins that
        # haven't migrated yet may still provide test/setup.py + docker-compose.yml.
        self.uses_adapter = (test_dir / 'adapter.py').exists()
        if not self.uses_adapter:
            setup_script = test_dir / 'setup.py'
            compose_file = test_dir / 'docker-compose.yml'
            if not setup_script.exists() or not compose_file.exists():
                raise ValueError(
                    f"Plugin test directory must contain either adapter.py (scenario-driven)\n"
                    f"or both setup.py and docker-compose.yml (legacy). Checked: {test_dir}"
                )
    ...
    def build_plugin(self, dry_run: bool = False) -> bool:
        ...
        # Adapter-based plugins have their kit artifacts merged by
        # generate_scenario.py at test time; legacy plugins still copy
        # artifacts into test/kits/ via their own setup.py.
    ...
    def clear_plugin_logs(self, dry_run: bool = False) -> bool:
        ...
        # Adapter-based plugins' logs live under generated/<scenario-id>/logs/,
        # which generate_scenario.py already wipes and recreates each run.
        if self.uses_adapter:
            print(f"  {colored('✓', GREEN)} Logs cleared (regenerated by scenario orchestrator)")
            return True
    ...
    def run_integration_test(self, wait_time: int = 10,
        ...
        if self.uses_adapter:
            # Convention: a plugin's default scenario is named "<plugin>-client-connect"
            # (see raceboat/test/integration/scenarios/); other scenarios for the
            # same plugin can be run directly via run_scenario.py --scenario-id.
            test_runner = Path(__file__).parent / 'run_scenario.py'
            command = [
                sys.executable, str(test_runner),
                '--scenario-id', f'{self.plugin_name}-client-connect',
