...
#ifndef _MockEncodingSdk
#define _MockEncodingSdk
...
class MockEncodingSdk : public IEncodingSdk {
public:
    ...
    MOCK_METHOD(ChannelResponse, onBytesEncoded,
                (RaceHandle handle, const std::vector<uint8_t> &bytes, EncodingStatus status),
                (override));

    MOCK_METHOD(ChannelResponse, onBytesDecoded,
                (RaceHandle handle, const std::vector<uint8_t> &bytes, EncodingStatus status),
                (override));
};
